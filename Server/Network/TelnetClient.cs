...
namespace Hedron.Network
{
    public class TelnetClient
    {
        ...
        public TelnetRetrievalData RetrieveInput()
        {
            ...
            if (streamreadbuffer != null)
            {
                ...
                if (streamreadbuffer.Length > TelnetConfig.MAX_NET_READS * BUF_SIZE)
                {
                    ...
                    return new TelnetRetrievalData(null, TelnetConfig.IO_READ.SENDEXCEED);
                }
                ...
                if (newlineindex == -1)
                {
                    return new TelnetRetrievalData(null, TelnetConfig.IO_READ.PENDINGREAD);
                }
                else
                {
                    if (newlineindex == 0)
                    ...
                    else
                    {
                        ...
                        // Process input
                        return new TelnetRetrievalData(Encoding.ASCII.GetString(inputlinebuffer), TelnetConfig.IO_READ.SUCCESSREAD);
                    }
                }
            }
        ...
        public void SendOutput(bool useColor)
        {
            if (_output != "")
            {
                if (useColor)
                {
                    _output = Formatter.FriendlyColorConversion(_output, false);
                    _output += Formatter.UniColorReset;
                }
                else
                {
                    _output = Formatter.FriendlyColorConversion(_output, true);
                }
