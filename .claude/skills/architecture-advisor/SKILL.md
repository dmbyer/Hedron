...
- **[roadmap/plan.md](../../../docs/roadmap/plan.md) (current focus + phases) · [roadmap/backlog.md](../../../docs/roadmap/backlog.md)** — what is scheduled and what is explicitly deferred (with rationale). A future you are weighing may already be tracked here.
...
2. **Find the seam.** Identify the new *verb*, *state*, or *signal* the feature introduces. For each, ask **where it belongs and what owns it**:
   ...
   - **Does this feature affect power, and how does its contribution enter the snapshot?** If the new state/mechanic should factor into `IPowerBudgetSystem`'s estimate (a new stat-like magnitude, a temporary buff stack, an equipped-ability roster), it never teaches the oracle a domain concept — it folds in as a new `ScoreId` weight, or a caller-summed estimated contribution mirroring `IEffectContributor`. See [`docs/design/power-model.md`](../../../docs/design/power-model.md); this is the snapshot-only extensibility rule, INV-2's power-specific instance. If the feature adds or moves a **tunable balance number**, or content that must land in a (Tier, Band) cell, point the plan at the balance catalog ([`docs/design/balance.md`](../../../docs/design/balance.md)) and the [`balance-tuning`](../balance-tuning/SKILL.md) skill — the catalog's maintenance contract names what the slice must update.
