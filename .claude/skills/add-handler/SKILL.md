...
Caller-initiated `SaveEntityAsync` is permitted only at **three** lifecycle boundaries (INV-22): (1) the **account/character creation** path (`LoginFlow`), called once after `AddComponent<PersistentEntity>` to make the new entity ID durable; (2) an **admin boundary save** — an admin-gated mutation command (e.g. `setplayer`, `setrespawn`) that saves once after mutating an already-persistent entity, paired with an audit event; and (3) a **session-end force-save** — a player logout/disconnect/`quit` saving the player as they leave. A handler never calls `SaveEntityAsync` for a runtime gameplay mutation — the **only** legitimate handler save site is the session-end/disconnect handler (`PlayerSessionHandler`); categories (1) and (2) live in commands/initiators. Admin content creation (`dig`, `mkitem`, `mkmob`, etc.) does **not** call `SaveEntityAsync` — the YAML file written by the command is the room/mob/item's sole durable state. World content carries no `PersistentEntity`.
