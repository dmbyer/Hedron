...
11. **Sync roadmap + disintegrate the plan.** Run the **sync-roadmap** skill. It updates `plan.md` (phase summary / current focus, if advanced), adds a row to `done.md`, creates `completed/<slug>.md`, then **distributes the plan's content into the living docs** (`features/`, `flows/`, `reference/`) and **deletes the plan** (disintegrate-on-ship, `INV-28`; the **manage-docs** skill has the placement rules). This is per-slice-loop step 7 ([`plan.md`](../../../docs/roadmap/plan.md)).
