...
- `PowerPerImprovement` (linear step), the growing-threshold curve, base awards, and anti-grind floor/cap all live in `ProgressionConstants` ([Category 3](../../../docs/architecture/05-configuration.md)). The **threshold curve is the "slowing rate" knob**; the power step stays **linear** — don't curve the power to slow gains, curve the threshold.
