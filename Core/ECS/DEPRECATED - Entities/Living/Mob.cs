	...
	public sealed class Mob : EntityAnimate
	{
		...
		{ 
			get 
			{
				...
				{
					EnsureMobComponentsInitialized();
					var mobDataComponent = EcsManager.World.GetComponent<MobDataComponent>(Instance.Value);
					if (mobDataComponent != null)
						return mobDataComponent.Behavior;
				}
				return _behavior;
			}
			set 
			{
				...
				if (Instance.HasValue)
				{
					...
					var mobDataComponent = EcsManager.World.GetComponent<MobDataComponent>(Instance.Value);
					if (mobDataComponent != null)
						mobDataComponent.Behavior = value;
				}
			}
		}

		private MobBehavior _behavior = new MobBehavior();

		/// <summary>
		/// The Effect for the mob's level
		/// </summary>
		[JsonProperty]
		private Effect LevelEffect 
		{ 
			get 
			{
				if (Instance.HasValue)
				{
					...
					var mobDataComponent = EcsManager.World.GetComponent<MobDataComponent>(Instance.Value);
					if (mobDataComponent != null)
						return mobDataComponent.LevelEffect;
				}
				return _levelEffect;
			}
			set 
			{
				...
				if (Instance.HasValue)
				{
					...
					var mobDataComponent = EcsManager.World.GetComponent<MobDataComponent>(Instance.Value);
					if (mobDataComponent != null)
						mobDataComponent.LevelEffect = value;
				}
			}
		}

		private Effect _levelEffect = new Effect();

		private MobLevel _level = MobLevel.Fair;
		...
		public Mob() 
			: base()
		{
			_levelEffect = Effect.NewMultiplier(MobLevelModifier.Map(MobLevel.Fair));
			ModifiedPools.CopyTo(CurrentPools);
		}

		/// <summary>
		/// Ensures mob-specific components are initialized if Instance is available
		/// </summary>
		private void EnsureMobComponentsInitialized()
		{
			if (Instance.HasValue && !EcsManager.World.HasComponent<MobDataComponent>(Instance.Value))
			{
				var world = EcsManager.World;
				
				world.AddComponent(Instance.Value, new MobDataComponent
				{
					Behavior = _behavior,
					Level = _level,
					LevelEffect = _levelEffect
				});
			}
		}
		...
		{
			_level = level;
			_levelEffect = Effect.NewMultiplier(MobLevelModifier.Map(level));
			ModifiedPools.CopyTo(CurrentPools);
		}
			...
			set
            {
				try
				{
					...
					if (Instance.HasValue)
					{
						...
						if (mobDataComponent != null)
						{
							mobDataComponent.Level = value;
							mobDataComponent.LevelEffect = LevelEffect;
						}
					}
				}
				catch
				{
					...
					if (Instance.HasValue)
					{
						...
						if (mobDataComponent != null)
						{
							mobDataComponent.Level = MobLevel.Fair;
							mobDataComponent.LevelEffect = LevelEffect;
						}
					}
				}
