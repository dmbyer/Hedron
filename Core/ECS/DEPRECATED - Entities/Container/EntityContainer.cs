...
namespace Core.ECS.Entities.Container
{
	...
	/// </summary>
	public class EntityContainer : Entity, IContainerObservable
	{
		...
		public int Count
		{
			get
			{
				...
				{
					EnsureContainerComponentsInitialized();
					var containerComponent = EcsManager.World.GetComponent<ContainerDataComponent>(Instance.Value);
					if (containerComponent != null)
						return containerComponent.Count;
				}
				return _entityList?.Count ?? 0;
			}
		}
		...
		/// <summary>
		/// Ensures container-specific components are initialized if Instance is available
		/// </summary>
		private void EnsureContainerComponentsInitialized()
		{
			if (Instance.HasValue && !EcsManager.World.HasComponent<ContainerDataComponent>(Instance.Value))
			{
				var world = EcsManager.World;
				
				world.AddComponent(Instance.Value, new ContainerDataComponent
				{
					EntityList = _entityList ?? new List<uint>(),
					Capacity = -1,
		...
		public void AddEntity<T>(uint? id, T entity, bool updatePersistence = true) where T: CacheableObject
		{
			...
			// Get the entity list from either component or legacy field
			List<uint> entityList;
			if (Instance.HasValue)
			{
				EnsureContainerComponentsInitialized();
				var containerComponent = EcsManager.World.GetComponent<ContainerDataComponent>(Instance.Value);
				entityList = containerComponent?.EntityList ?? _entityList;
			}
			else
			{
			...
			if (entityList.Contains((uint)id) && CacheType == CacheType.Instance)
				throw new ArgumentException($"Cannot add duplicate instance to {nameof(EntityContainer)}", nameof(id));
			else
			{
				entityList.Add((uint)id);
				
				// Update legacy field as well for backward compatibility
		...
		private void RemoveEntity(uint? id, CacheableObject entity, bool updatePersistence)
		{
			...
			// Remove from both component and legacy field
			if (Instance.HasValue)
			{
				EnsureContainerComponentsInitialized();
				var containerComponent = EcsManager.World.GetComponent<ContainerDataComponent>(Instance.Value);
				containerComponent?.EntityList.Remove((uint)id);
			}
		...
		protected override void OnCacheObjectRemoved(object source, CacheObjectEventArgs args)
		{
			...
			if (Instance.HasValue)
			{
				EnsureContainerComponentsInitialized();
				var containerComponent = EcsManager.World.GetComponent<ContainerDataComponent>(Instance.Value);
				containerComponent?.EntityList.Remove(args.ID);
			}
		...
		protected override void OnObjectDestroyed(object source, CacheObjectEventArgs args)
		{
			if (args.CacheType == CacheType.Instance)
			{
				...
				DataAccess.RemoveMany(entityList, args.CacheType);
			}
		}
