...
```
Registered as a singleton in `BalanceInspectionModule.AddBalanceInspectionModule` (called from `CompositionRoot`, not `Program.cs`, so the `Hedron.Web` host can resolve it for the editor readout). Implemented (power-budget-inspector slice prog-3; Tier×Band revision + recalibration prog-3b).
...
**Purpose:** Bulk band-drift audit: `Audit()` enumerates every authored item/mob template (`ITemplateRegistry.AllBlueprintIds`), projects each via the shared item/mob power-projection seam, classifies via `IPowerBudgetSystem`, and returns a `BalanceAuditReport` — every item/mob whose global band-index drift (`|index(authored) − index(computed)|`, `index(T,B) = T·BandsPerTier + (B−1)`) exceeds `BalanceAuditConstants.BandDriftTolerance` (authored `Band = 0` excluded from drift, but still bucketed), plus counts bucketed by computed `(Tier, Band)`. Recomputed on demand, never cached/persisted (INV-24 spirit); a soft/advisory tool consumed by the Blazor Integrity page — never a build/reload/CI gate.
**Location:** `Core/Modules/BalanceInspection/Systems/IBalanceAuditSystem.cs` · `BalanceAuditSystem.cs` · `BalanceAuditReport.cs` · `BalanceAuditConstants.cs`
**Dependencies:** `ITemplateRegistry`, `IPowerBudgetSystem`, `IItemPowerProjectionSystem`, `IMobPowerProjectionSystem`.
Registered as a singleton in `BalanceInspectionModule.AddBalanceInspectionModule`. Implemented (power-model-revision slice prog-3b).
