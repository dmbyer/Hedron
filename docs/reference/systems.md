...
### IContentValidator
**Purpose:** On-demand content validator, factored out of `RegistryValidationBootstrap` so the same referential-integrity rules run in two call modes. `ValidateRegistry` is the whole-registry sweep (ability→effect/aspect cross-refs, aspect-composition normalization, starting-ability cross-refs, live area-entity affinity normalization, and a warn-not-error room-coordinate-collision check sourced from `ITemplateRegistry`) used at boot. `Validate(IEntityTemplate)` is the single in-memory definition check used by the authoring editor per edit, the JSON endpoints, and the bulk generator pre-write; per-kind rules today are the **area** aspect-affinity composition check and the **mob** currency-loot check (each authored range non-negative and `Min ≤ Max` — fail-fast, refused rather than clamped, since an inverted range makes the spawn-time roll undefined; added authoring-api-surface WP1, replacing a partial clamp in `MobEditor.razor` so every writing surface gets the rule). `ValidateBlueprintId(ContentKind, string)` validates a candidate blueprint id — shared by `IContentDefinitionCatalog.RenameAsync` and the create-with-id guard: format errors (empty, path separator, `..` segment, a character outside `[A-Za-z0-9._-]`, a reserved Windows device name) refuse; a kind-prefix mismatch (e.g. a room id not starting with `room.`) is a non-blocking warning — the loader keys off the kind subdirectory, not the prefix. Returns a structured `ValidationReport` (`Errors` + `Warnings`; `IsValid` is errors-only) and **never throws** (INV-5) — the host decides fail-fast policy, logging warnings without aborting.
...
**Interface:** [`IContentValidator.cs`](../../Core/Modules/World/Systems/IContentValidator.cs) — `ValidateRegistry(startingAbilityIds)` / `Validate(IEntityTemplate)` / `ValidateBlueprintId(ContentKind, string)`. Registered as a singleton in `WorldModule.AddWorldModule`. Consumed by `RegistryValidationBootstrap` (boot), `IContentDefinitionCatalog.SaveAsync` (per-edit), and `IContentDefinitionCatalog.RenameAsync`/`CreateAsync` (id validation). Implemented (Phase 3 content-tooling WP-1; coordinate-collision warning added world-editor-grid WP1; `ValidateBlueprintId` added blueprint-id-editing WP1).

### IContentReferenceIndex (Authoring module)
**Purpose:** Declared-edge reference model over the on-disk YAML definition set. Answers three read questions without applying any policy: *does this target exist?*, *who points at this id?*, and *what is broken across all definitions?* Pure read — returns structured results, publishes nothing, holds no live entities (INV-5). The declared edge set drives all consumers (delete-cascade, rename-cascade, warn-but-allow save, integrity sweep, and save-time cross-ref check) without per-edge code paths (INV-19).
...
Declared edges (six total; adding a new edge is a one-line data change — INV-19). Each edge declares one or more target kinds (`EdgeDeclaration.TargetKinds`); `Referrers` matches when the searched kind is *in* the set (contains-match), while `SweepBroken`/`BrokenFor` flag broken only when the id resolves against *none* of the declared target kinds:
- `(Room, AreaId) → Area`
...
- `(Room, SpawnRules[].BlueprintId) → {Mob, Item}` — **two-kind target**; one tuple per spawn rule, labeled `SpawnRules[{id}]` (encodes the id, not a list index, so cascade rewrites are stable/idempotent). A `SpawnRule` id carries no kind discriminator, so it resolves against either kind — the pathological case of a mob file and an item file sharing an id is a bounded, accepted over-match (harmless for rename/delete cascades). Added blueprint-id-editing WP1, closing a pre-existing gap where deleting a mob/item left dangling `Room.SpawnRules` entries invisible to the sweep.
...
**Purpose:** The shared content-definition layer both content-tooling tracks call — the offline Blazor editor and the headless bulk generator. Reads/lists/loads/creates/validates/writes/deletes/renames the YAML content-definition families (area, room, item, mob). **Writes and deletes YAML only** — never creates or destroys a live entity, adds `PersistentEntity`, or calls `SaveEntityAsync` ([INV-12](../architecture/checklist.md)/[INV-22/23](../architecture/checklist.md)); applying content changes to the live world is a separate `reload` step. `SaveAsync` validates before writing. `CreateNew` mints an ad-hoc blueprint id (or accepts a deliberate one) without touching the registry or the world; `CreateAsync` is the create-guarded write; `RenameAsync` changes a definition's blueprint id and cascades the change through every YAML reference. `WithBlueprintId` and `CreateNextFrom` are pure authoring-form helpers (rekey-in-place, and the per-kind carry-forward policy for "save and create next"). **Reads are cached** — see *Caching and concurrency* below.
...
**Dependencies:** `IContentSerializer`, `IContentValidator`, `ITemplateRegistry`, `IContentReferenceIndex`, per-kind content writers, `IOptions<WorldOptions>`, `ILogger`, `IContentFileReader` (optional — the catalog-scoped read seam; defaults to direct `System.IO` when omitted).
**Interface:** [`IContentDefinitionCatalog.cs`](../../Core/Modules/Authoring/Systems/IContentDefinitionCatalog.cs) — `List` / `RoomsInArea` / `Load` / `SaveAsync` / `SaveRoomAsync` / `DeleteAsync` / `RenameAsync` / `CreateNew` (two overloads) / `CreateAsync` / `RemoveRoomExitAsync` / `WithBlueprintId` / `CreateNextFrom` / `Invalidate`. Registered as a singleton in `AuthoringModule.AddAuthoringModule`. Implemented (Phase 3 content-tooling WP-1; area-association read-model added in Slice A; `Delete`, warn-but-allow save, bidirectional room save added in Slice B WP-2; `RemoveRoomExitAsync` added world-editor-grid WP1; `RenameAsync`/`CreateAsync`/choose-at-creation added blueprint-id-editing WP1; caching + `WithBlueprintId`/`CreateNextFrom`/`Invalidate` added authoring-editor-repair). The Blazor host, the bulk-generation system, and the visual grid area editor are thin callers. See [`../features/admin-authoring/content-tooling.md`](../features/admin-authoring/content-tooling.md) for the full design.
...
The two-hop resolution uses a room→area map **derived from the cached room summaries** — so listing all four kinds sweeps each kind's directory at most once per invalidation, and item/mob area resolution stays O(1) per entry.
...
**Caching and concurrency.** Reads are served from an in-memory index: per-kind summary lists and the derived room→area map are corpus sweeps; **the per-id definition map fills one id at a time, on demand**. That granularity is load-bearing — corpus-populated per-id caching plus whole-index invalidation would turn `ITemplateConformanceSystem.ApplyFlaggedAsync`'s and `IAreaLayoutSystem.ApplyProposalAsync`'s per-entry `Load`→write loops into N full sweeps; as built, a `Load` after an invalidation is one file read and both loops stay O(N). **Every mutator invalidates the whole index** (`SaveAsync`, `SaveRoomAsync`, `CreateAsync`, `DeleteAsync`, `RenameAsync`, `RemoveRoomExitAsync`) because writes cascade across files, so entry-scoped invalidation cannot express them. The per-id map caches file *text*, not templates — `Load` deserializes per call, since callers mutate what they get back.

Concurrency posture ([INV-31](../architecture/checklist.md)), **posture: guard** — two guards, for two different things.

*Index consistency.* The catalog is a DI singleton reached concurrently from multiple Blazor circuits. Every mutator is `async` and invalidates *after* an awaited write, so a thread-affine `ReaderWriterLockSlim` is unusable (it cannot be held across an `await`). The index is a snapshot object swapped under a plain `lock`; readers take the reference with no lock and populate lazily into it, and snapshot identity carries the generation so a sweep that began before a concurrent write cannot republish pre-write state. Two consequences worth stating: the one-sweep-per-invalidation bound is **per reader** (concurrent cold readers may duplicate a sweep — benign, last publish wins), and a sustained write loop can invalidate faster than a concurrent reader publishes, leaving that reader re-sweeping. Both are deliberate: correctness over cache retention while writes are in flight.

*Write serialization* (added authoring-api-surface WP2, when the authoring host's JSON endpoints made request threads a **second concurrent writer** alongside the circuits). Every public mutator runs under one `SemaphoreSlim(1,1)` — a guard, and an async-compatible one for the same no-lock-across-`await` reason. The critical section spans the write cascade **and** the index invalidation that ends it. This also closes a real TOCTOU in `CreateAsync`, which checks `Resolves` and then writes non-atomically; concurrent creates of one id now admit exactly one winner. **Re-entrancy is load-bearing:** `CreateAsync` is defined in terms of `SaveAsync`, so a semaphore taken inside every public mutator self-deadlocks on `create`. The shape is a private `*Core` per mutator that calls only other `*Core` methods, plus a public wrapper that takes the gate exactly once — never call a public mutator from inside the gate.

Three things *outside* the catalog are what make its lock-free read side actually safe under that concurrency, each a live hazard once request threads read alongside a circuit:

1. The `I*ContentWriter` family publishes through `AtomicFileWrite` — write a `.tmp`, then **`File.Replace`** the destination — so no reader observes a truncated file.
2. `ContentFileReader` reads with `FileShare.ReadWrite | FileShare.Delete`.
3. Each `ITemplateDeserializer` holds its YamlDotNet deserializer **thread-locally**, since YamlDotNet's is not thread-safe and a shared instance corrupts its type cache under concurrent reads.

(1) and (2) are **jointly** necessary and neither works alone — measured on Windows with a reader holding the destination: `File.Move(overwrite: true)` fails under *both* `FileShare.Read` and `ReadWrite|Delete` (it cannot rename over a delete-pending file however it was opened); `File.Replace` fails under `FileShare.Read` and succeeds only under `ReadWrite|Delete`. A retry backstop exists in `AtomicFileWrite` for scanner/indexer interference, but it is not the mechanism — the concurrent-reader case is handled correctly, not merely retried past. Weakening any of the three re-breaks concurrent reads.

Neither guard makes the **multi-file cascade** transactional: a reader can still observe a mid-cascade state where one file of a delete/rename has been rewritten and another has not, and a mid-cascade failure still leaves a partial edit. That is the pre-existing, [backlog](../roadmap/backlog.md)-tracked debt, unchanged.

Writes that bypass the catalog — the `generate` CLI in its own process, the game host's `mk*`/`set*`/`dig` verbs writing through `I*ContentWriter` — do not invalidate a running editor's index. `Invalidate()` is the explicit escape hatch (surfaced as the content browser's **Refresh from disk** action); the residual staleness is recorded in [backlog.md](../roadmap/backlog.md).

`WithBlueprintId(definition, blueprintId)` returns the definition with only its id replaced — every other authored field preserved, self-referential ids rewritten by the same `CloneWithNewId` rule `RenameAsync` uses (one id-rewrite rule, not two). A blank id falls back to a freshly minted ad-hoc id. This is what the four editors' `OnBlueprintIdChanged` calls; calling `CreateNew` there instead silently discards the in-progress form, and an architecture guard now forbids it.

`CreateNextFrom(previous, name)` mints the next definition in a "save and create next" run, delegating id minting to `CreateNew` (no third id-minting path). Carry-forward is authoring policy plus kind dispatch, so it lives here rather than in a component: **Area** carries nothing; **Room** carries `AreaId`; **Item** carries Tier, Band, `ItemType`, `WornSlots`; **Mob** carries Tier, Band, `SpawnRoomBlueprintId`. Everything else resets to a fresh definition's defaults.

*(`IContentFileReader` is an infrastructure port, not a domain system — no row here by [INV-16](../architecture/checklist.md)/[INV-29](../architecture/checklist.md). It wraps the catalog's directory listings and file reads so a test can count them deterministically, and is registered in `AuthoringModule` for the composition-root smoke guard.)*
...
`RenameAsync(kind, oldId, newId, ct)` changes a definition's blueprint id and cascades the change through the content graph. `IContentValidator.ValidateBlueprintId(kind, newId)` runs first — format errors refuse with no write; a kind-prefix mismatch is a deferred warning. `IContentReferenceIndex.Resolves(kind, newId)` then guards uniqueness — a collision refuses with no write (no merge). On pass, a fresh template is constructed with `BlueprintId = newId` (template ids are get-only/constructor-set; every field is copied and the definition's own self-referential fields — e.g. a self-loop exit — are rewritten `oldId → newId` in the same pass), written via the matching writer, then every *external* referrer found via `Referrers(kind, oldId)` (the target's own self-reference is excluded — already handled) is rewritten by the same `TryApplyCascadeAsync` helper `DeleteAsync` uses, passing `newId` instead of `null`. The `oldId` file is then deleted. Out-of-YAML advisories are folded into `Warnings`: renaming a room always warns that persistent player/item locations re-key on the next `reload`; renaming the room matching `WorldOptions.StartingRoomBlueprintId` adds a second warning to update `appsettings.json` by hand. **YAML-file operation only** — no `EntityService`, no SQLite, no live-world mutation (INV-22/23); best-effort per referrer, matching `DeleteAsync`. Returns `ContentRenameResult`. Added blueprint-id-editing WP1.

`CreateNew(kind, name, string? blueprintId)` overload — when `blueprintId` is non-null/non-empty, the definition carries that deliberate id; when null/empty, falls back to the existing `AdhocBlueprintId.Generate` path. `CreateAsync(definition, ct)` is the create-guarded write: runs `ValidateBlueprintId` + a `Resolves` uniqueness refusal before delegating to `SaveAsync` — the guard a deliberate id needs that adhoc ids never required (they never collide by construction). Edits stay on the unguarded `SaveAsync` (overwrite-on-edit, contract unchanged). Added blueprint-id-editing WP1.

`ContentRenameResult` shape: `record(bool Success, string OldPath, string NewPath, string OldBlueprintId, string NewBlueprintId, IReadOnlyList<ReferrerEdit> CascadeEdits, IReadOnlyList<string> Warnings, IReadOnlyList<string> Errors)` (`Core/Modules/Authoring/ContentRenameResult.cs`) — modeled on `ContentDeleteResult` (the cascade edits) fused with `ContentWriteResult` (success/errors/warnings, because a rename can be refused). Factories: `Ok(oldPath, newPath, oldId, newId, cascadeEdits, warnings)` · `Failed(oldId, newId, errors)`.
...
public interface IAreaLayoutSystem
{
    ...
    Task<AreaConnectResult> ConnectAsync(
        string fromRoomBlueprintId, string toRoomBlueprintId, CancellationToken ct = default);
}
...
public enum AreaConnectOutcome { Connected, RoomNotFound, NotAdjacent, WriteFailed }
...
```
`ConnectAsync(from, to)` owns the grid's **connect policy** — which direction pair to write, that the write is bidirectional, and what counts as connectable. It derives the direction from the two rooms' laid-out positions (authored coordinates, or a coordless room's position in this area's `Propose` proposal, so the gesture agrees with the ghost cell the author sees), then writes that exit through `SaveRoomAsync(bidirectional: true)` and inherits its conflict policy. Adjacency is judged in all three axes: a room directly above another yields `Up`, and two rooms on different Z layers are never connected as though on one. A refusal is a **result, not an exception** — `NotAdjacent` (including a target laid out in another area, whose grid is a separate coordinate space) and `RoomNotFound` both mean "not a connect gesture", which the grid renders as a selection change. Added authoring-api-surface WP1, extracting the decision out of `AreaGridEditor.razor`; the direction *arithmetic* was already the tested Core helper `DirectionExtensions.FromOffset`, and an architecture guard now forbids that helper inside `Hedron.Web/`.

Registered as a singleton in `AuthoringModule.AddAuthoringModule`. Consumed by `AreaGridEditor.razor`. Implemented (world-editor-grid WP2; `ConnectAsync` added authoring-api-surface WP1).
...
**Interface:** [`IItemBuilderSystem.cs`](../../Core/Modules/Items/Systems/IItemBuilderSystem.cs) — `CreateItem` / `SetItemName` / `SetItemDescription` / `SetItemKeywords` / `SetItemType` / `SetItemSlots` / `SetItemStatBonus` / `ClearItemStatBonuses` / `SetItemValue` / `SetItemTier` / `SetItemBand`. Returns `ItemCreationResult(ItemEntityId, BlueprintId, Template)`; never calls persistence or events. `SetItemStatBonus` add-or-replaces one `(ScoreId, magnitude)` row (magnitude 0 removes it); both it and `SetItemSlots` mirror changes to `ItemDataComponent` **and** `ItemTemplate` so the assignment survives `@reload`. `SetItemValue` sets the item's base-unit Coin `Value` (`long`, default 0) with the same dual-write to `ItemDataComponent` and `ItemTemplate`; it is a pure setter (the command rejects negatives at the edge). `SetItemTier`/`SetItemBand` (prog-3b, replacing the one-axis `SetItemBand`) dual-write `ItemDataComponent.Tier`/`Band` and `ItemTemplate.Tier`/`Band`; the command range-validates (tier 0–6, band 0–3) before calling. See [`../features/items/item-inventory-system.md`](../features/items/item-inventory-system.md) for the authoring flow. Implemented (Phase 3 slices 6, 7, 9-c; `StatBonuses` in wearable-equipment-expansion; Tier×Band split prog-3b).
...
**Interface:** [`IMobBuilderSystem.cs`](../../Core/Modules/Mobs/Systems/IMobBuilderSystem.cs) — `CreateMob` / `SetMobName` / `SetMobDescription` / `SetMobKeywords` / `SetMobType` / `SetAttribute` / `SetMobProtection` / `SetMobTier` / `SetMobBand`. `CreateMob` generates a unique blueprint id (`mob.adhoc.<8-char-base36>`), creates the entity, attaches `MobDataComponent` + `BlueprintComponent` + `LocationComponent { RoomEntityId }`, and registers a minimal `MobTemplate`. Slice 8-a added `SetAttribute` for `AttributesComponent`/`PoolsComponent` mutations; enforces `CurrentX ≤ MaxX` clamp (INV-8). Slice 12b (WP-2) added `SetMobProtection(mobEntityId, flags)`: dual-writes `ProtectionComponent` on the live entity and `MobTemplate.Protection`; removes the component when `flags == None`. `SetMobTier`/`SetMobBand` (prog-3b, replacing the one-axis `SetMobBand`) dual-write `MobDataComponent.Tier`/`Band` and `MobTemplate.Tier`/`Band`; the command range-validates (tier 0–6, band 0–3) before calling. See [`../features/mobs/mob-system.md`](../features/mobs/mob-system.md) for the full builder model and YAML shape. Implemented (Phase 3 slices 8, 8-a, 9-d, 12b; Tier×Band split prog-3b).
...
**Location:** `Core/Systems/IPowerBudgetSystem.cs` · `PowerBudgetSystem.cs` · `PowerBudgetMath.cs` · `PowerBudgetTunables.cs` · `PowerSnapshot.cs` · `PowerBand.cs` · `PowerRange.cs`
**Dependencies:** exactly one constructor parameter — `PowerBudgetTunables`, a plain-data record (no service/domain type). The oracle still imports no `Core/Modules/<Feature>/` domain type beyond the allowlisted `Stats` (`ScoreId`) — never a reference into the domain `Account` or `Ascension` modules directly (INV-2). `PowerBudgetTunables.Default` is the compiled fallback (pre-sim-1 constant values); production composes the tunables from the `IBalanceStandardsRegistry` instead (sim-1).
```csharp
...
```
**`PowerBudgetMath` — the pure split** (authoring-api-surface WP1). The formulas themselves are `public static` methods over a *caller-supplied* `PowerBudgetTunables`: `Estimate(tunables, snapshot, tier)` · `Classify(tunables, power)` · `TargetRange(tunables, tier, band)` · `BandAnchor(tunables, tier)`. `PowerBudgetSystem` is a thin instance facade that binds them to the snapshot the host composed at startup, so there is one home per formula (INV-27) and the interface above is unchanged. Call `PowerBudgetMath` directly **only** when evaluating against tunables other than the composed ones — the balance-standards editor previewing an unsaved candidate document per (Tier, Band) cell is the sole such caller, and it hoists the tunables composition out of the per-cell loop. This is deliberately *not* an `IPowerBudgetSystem` overload: such a member would ignore instance state on every implementation and force a caller with no use for the composed snapshot to inject the singleton anyway. It also leaves the composed instance's ctor-injected snapshot semantics untouched (a recorded decision — see [backlog.md](../roadmap/backlog.md) §Live balance-standards reload). An architecture guard forbids `new PowerBudgetSystem(` inside `Hedron.Web/`.

Registered as a singleton in `BalanceInspectionModule.AddBalanceInspectionModule` (called from `CompositionRoot`, not `Program.cs`, so the `Hedron.Web` host can resolve it for the editor readout). Implemented (power-budget-inspector slice prog-3; Tier×Band revision + recalibration prog-3b; tunables-as-injected-data promotion sim-1; pure `PowerBudgetMath` split authoring-api-surface WP1).

### ItemPowerProjectionSystem / IItemPowerProjectionSystem (Items module)
**Purpose:** Domain-tier shared projection seam (INV-19): turns an authored `ItemTemplate` or a live `ItemDataComponent` into the `PowerSnapshot` `IPowerBudgetSystem` estimates from — both overloads key on `StatBonuses`, the item's only power-relevant data. Replaces three previously hand-rolled inline builds (`PowerCommand`'s item path, `ItemEditor`, the audit).
**Location:** `Core/Modules/Items/Systems/IItemPowerProjectionSystem.cs` · `ItemPowerProjectionSystem.cs`
**Dependencies:** none.
Registered as a singleton in `ItemsModule.AddItemsModule`. Implemented (power-model-revision slice prog-3b).

### MobPowerProjectionSystem / IMobPowerProjectionSystem (Mobs module)
**Purpose:** Domain-tier shared projection seam (INV-19): turns an authored `MobTemplate` into the `PowerSnapshot` `IPowerBudgetSystem` estimates from (attrs/pools plus the `IStatSystem`-mirroring derivations `AttackPower = Body/2`, `Defense = Body/4`). Template-sourced only — `PowerCommand`'s live self/mob path keeps its own `IStatSystem` snapshot, a distinct correct projection not folded in here. Consumed by `MobEditor` and `IBalanceAuditSystem`.
**Location:** `Core/Modules/Mobs/Systems/IMobPowerProjectionSystem.cs` · `MobPowerProjectionSystem.cs`
**Dependencies:** none.
Registered as a singleton in `MobsModule.AddMobsModule`. Implemented (power-model-revision slice prog-3b).

### MobPowerReadoutSystem / IMobPowerReadoutSystem (BalanceInspection module)
**Purpose:** The **per-template** counterpart to `IBalanceAuditSystem`'s corpus sweep: `Read(MobTemplate)` composes the shared mob projection seam, the core-tier oracle, and the standards registry's drift tolerance into the single `MobPowerReadout` an editor renders beside an authored stat block — estimated `Power`, the `Computed` (Tier, Band) cell, the authored tier/band echoed back, the authored cell's `AuthoredTargetRange`, and `DriftsFromAuthoredCell`. Template-sourced only; pure — returns a result, writes nothing, publishes nothing (INV-5/INV-12).

`AuthoredTargetRange` is `null` when the authored cell has no window: band `0` (unbanded, the default) or a tier/band outside the standards table — a readout is a display, not a place to surface the oracle's fail-fast `ArgumentOutOfRangeException`. `DriftsFromAuthoredCell` is always `false` for an unbanded mob: authored band `0` is "unbanded", not "band zero", and makes no claim to compare against — the same exclusion `IBalanceAuditSystem`'s sweep applies.

**Location:** `Core/Modules/BalanceInspection/Systems/IMobPowerReadoutSystem.cs` · `MobPowerReadoutSystem.cs`
**Dependencies:** `IPowerBudgetSystem`, `IMobPowerProjectionSystem`, `IBalanceStandardsRegistry`.
```csharp
...
public sealed record MobPowerReadout(
    int Power, PowerBand Computed, int AuthoredTier, int AuthoredBand,
    PowerRange? AuthoredTargetRange, bool DriftsFromAuthoredCell);
```
**Why it exists:** two surfaces now show that readout — `MobEditor.razor` and the authoring API's `POST /api/power/mob` — and a second hand-composition of oracle + projection + tolerance in an entry-point surface is exactly the duplication INV-19 exists to stop. Registered as a singleton in `BalanceInspectionModule.AddBalanceInspectionModule`. Implemented (authoring-api-surface WP2).

### BalanceAuditSystem / IBalanceAuditSystem (BalanceInspection module)
**Purpose:** Bulk band-drift audit: `Audit()` enumerates every authored item/mob template (`ITemplateRegistry.AllBlueprintIds`), projects each via the shared item/mob power-projection seam, classifies via `IPowerBudgetSystem`, and returns a `BalanceAuditReport` — every item/mob whose global band-index drift (`|index(authored) − index(computed)|`, `index(T,B) = T·BandsPerTier + (B−1)`, computed by `PowerBudgetTunables.GlobalBandIndex`) exceeds its injected `bandDriftTolerance` (authored `Band = 0` excluded from drift, but still bucketed), plus counts bucketed by computed `(Tier, Band)`. Recomputed on demand, never cached/persisted (INV-24 spirit); a soft/advisory tool consumed by the Blazor Integrity page — never a build/reload/CI gate.
**Location:** `Core/Modules/BalanceInspection/Systems/IBalanceAuditSystem.cs` · `BalanceAuditSystem.cs` · `BalanceAuditReport.cs`
**Dependencies:** `ITemplateRegistry`, `IPowerBudgetSystem`, `IItemPowerProjectionSystem`, `IMobPowerProjectionSystem`, `PowerBudgetTunables`, `int bandDriftTolerance` (both plain-data constructor params, sourced from `IBalanceStandardsRegistry` by the DI factory — see below; formerly the standalone `BalanceAuditConstants`, retired sim-1).
Registered as a singleton in `BalanceInspectionModule.AddBalanceInspectionModule`. Implemented (power-model-revision slice prog-3b; tolerance/index math moved onto the standards registry sim-1).

### TemplateConformanceSystem / ITemplateConformanceSystem (BalanceInspection module)
**Purpose:** The fitter (sim-5) that closes the balance workbench's observation→correction loop `IBalanceAuditSystem` opens: `Preview(kind, blueprintId)` loads a template **from disk** via `IContentDefinitionCatalog.Load` (never the audit's boot-time registry), and — unless it already classifies on its authored (Tier, Band) target — computes a closed-form uniform scale factor toward the target cell's midpoint (`k = (targetMid − tierTerm) / variablePower`, exploiting `IPowerBudgetSystem.Estimate`'s weighted-sum linearity), rounds each field half-away-from-zero, then **verifies with the real projection seam** and applies a bounded ±1 largest-measured-contribution correction (cap 8 iterations) if the rounded result still misses the target cell. Returns `Fitted` (with a per-field before/after diff), `AlreadyInRange`, or `NotFittable` (`ZeroWeightedPowerVector` / `UnbandedTemplate` / `RoundingDidNotConverge`) — never invents a stat block from a range (scaling, not synthesis). `ApplyAsync` **re-derives the fit from disk** (idempotent — never trusts a prior `Preview`) and, only for `Fitted`, calls `IContentDefinitionCatalog.SaveAsync` (the identical validate-then-write, warn-but-allow path the offline editors use). `PreviewFlagged`/`ApplyFlaggedAsync` loop the single-item path over `IBalanceAuditSystem.Audit().Drifted` — one code path for single and bulk (INV-19). Item knobs: `StatBonuses` magnitudes only. Mob knobs: `Mind`/`Body`/`Spirit`/`Attunement`/`MaxHp`/`MaxMana`/`MaxStamina`/`MaxAstra` — exactly the fields the item/mob power-projection seams read; derived `AttackPower`/`Defense` follow scaled `Body` through the real projection. YAML-side only — no live entity, `EntityService`, `PersistentEntity`, or bus publish anywhere (INV-5/12/21/22/23); live worlds pick up a conformed template on the existing `reload` step.
**Location:** `Core/Modules/BalanceInspection/Systems/ITemplateConformanceSystem.cs` · `TemplateConformanceSystem.cs` · `Core/Modules/BalanceInspection/ConformanceReport.cs` (`ConformancePreview`/`ConformanceFieldChange`/`ConformanceApplyResult`/`ConformanceBulkResult`/`ConformanceStatus`/`ConformanceNotFittableReason`)
**Dependencies:** `IContentDefinitionCatalog`, `IPowerBudgetSystem`, `IItemPowerProjectionSystem`, `IMobPowerProjectionSystem`, `IBalanceAuditSystem` — exactly these five (ctor-shape guard-tested; precedent `PowerBudgetSystem_has_no_domain_module_dependency`).
Registered as a singleton in `BalanceInspectionModule.AddBalanceInspectionModule`. Consumed by the Blazor Integrity page (per-row and bulk preview/apply). Implemented (conformance-tooling slice sim-5).

### IBalanceStandardsStore / BalanceStandardsStore (BalanceInspection module — Standards)
**Purpose:** Domain-tier (may legally import `Ascension`/`Account`/`Abilities`, unlike the core-tier oracle) YAML load/validate/save for the balance-standards document (Spine F registry backing store, sim-1): `Load()` reads `Balance:StandardsPath` (default `data/balance/standards.yaml`), falls back to `BalanceStandardsDefaults.Document` if absent, runs structural fail-fast validation (unknown score id, out-of-range/duplicate cell, `BandSpan` calibration violation, negative tolerance — throws `InvalidOperationException` naming the file and violation), then mirror-drift validation against `AscensionConstants`/`CharacterDefaultsOptions` and unknown-ability-kit-id checks against `IAbilityRegistry` (returns warnings, never silently absorbed). `SaveAsync(document)` re-validates structurally (refuses the write on failure, no partial file) then writes atomically (tmp → rename), returning the same warning set on a structural pass (warn-but-allow). Deliberately outside `IContentDefinitionCatalog` (seed OQ1) — a single-document criteria file doesn't fit the catalog's per-blueprint `IEntityTemplate`/spawn/delete-cascade semantics — but mirrors its validate-then-write posture.
**Location:** `Core/Modules/BalanceInspection/Standards/IBalanceStandardsStore.cs` (interface + `BalanceStandardsSaveResult`) · `BalanceStandardsStore.cs` (implementation) · `BalanceStandardsDocument.cs` (`BalanceStandardsDocument`/`BalanceStandard`/`ReferenceBuildDefinition`/`OutcomeTolerances`) · `BalanceStandardsDefaults.cs`
**Dependencies:** `IOptions<BalanceOptions>`, `IOptions<CharacterDefaultsOptions>`, `IAbilityRegistry`.
```csharp
...
```
Registered as a singleton in `BalanceInspectionModule.AddBalanceInspectionModule`. Consumed by the `IBalanceStandardsRegistry` load-once factory (boot) and the Blazor Standards page (author/save). Implemented (balance-standards-registry slice sim-1).

### IBalanceStandardsRegistry / BalanceStandardsRegistry (BalanceInspection module — Standards)
**Purpose:** The loaded, dense-filled balance-standards table — the criteria seam every consumer reads. `IRegistry<PowerBand, BalanceStandard>` over every (Tier, Band) cell in `[0, Tunables.MaxTier] × [1, Tunables.BandsPerTier]`; sparse-authored cells (from the loaded document) fill with an empty-gear reference build and the global outcome tolerances at construction (seed OQ4 — a criteria file need not author every cell). `Tunables` is the `PowerBudgetTunables` the oracle is composed from; `BandDriftTolerance` replaces the former compiled constant; `OutcomesFor(tier, band)` prefers a cell's `OutcomesOverride`, falling back to the document's global tolerances; `ReferenceSnapshot(tier, band)` = `Tunables.ReferenceBaseScores + cell.ReferenceBuild.GearBonuses` (the tier baseline enters via `IPowerBudgetSystem.Estimate`'s tier argument, exactly as live snapshots do).
**Location:** `Core/Modules/BalanceInspection/Standards/IBalanceStandardsRegistry.cs` · `BalanceStandardsRegistry.cs`
**Dependencies:** none beyond the `BalanceStandardsDocument` passed to its constructor.
```csharp
...
```
Registered as a singleton **load-once factory** in `BalanceInspectionModule.AddBalanceInspectionModule`: resolves `IBalanceStandardsStore.Load()` exactly once, logs returned warnings, and is the composition source for the `PowerBudgetTunables` singleton `PowerBudgetSystem` and `BalanceAuditSystem` are constructed from. Eagerly resolved at boot via `RegistryValidationBootstrap`'s constructor dependency (both hosts) — see [Flow 1](../architecture/flows/flow-01-server-startup.md). Implemented (balance-standards-registry slice sim-1).
...
**Purpose:** YAML load + fail-fast structural validation of a `ScenarioDefinition` (kind, name, seed, iterations, maxTicksPerRun, sides→combatants, + a kind-gated `progression:` section). Posture mirrors `BalanceStandardsStore`: validate-then-use, every violation named in one thrown `InvalidOperationException`. `Validate` is also callable directly on an in-memory definition (the sim-3 editor and a future generator candidate-check reuse it without a disk round-trip). The known-policy-id set is DI-collected from `IEnumerable<ISimCombatantPolicy>`, never a hardcoded list — **sim-4:** policy-id validation is Combat-only (a `progressionRate` side never chooses actions; `policyId` may be omitted). **sim-3 addition:** `SaveAsync` validates then atomically writes a scenario as camelCase YAML into `Simulation:ScenarioDirectory` (upsert by sanitized name — a re-save with the same `Name` overwrites its file); `List` enumerates saved scenario files for the editor's load dropdown. A page-saved scenario is `simulate --scenario <path>`-runnable with no changes — one store, one YAML dialect. **sim-4 addition:** for `ScenarioKind.ProgressionRate`, `progression:` is required (missing → named violation) and validated — `targetTrack` must be one of `ProgressionConstants.CombatTracks`, `targetImprovements`/`maxKillsPerRun` must be positive, `ticksPerKill` (when present) must be positive; the section is rejected on a `Combat` scenario. Both shipped kinds require exactly 2 sides of exactly 1 combatant each.
**Location:** `Core/Modules/Simulation/Systems/ISimScenarioStore.cs` · `SimScenarioStore.cs` · `Core/Modules/Simulation/ScenarioDefinition.cs` (`ProgressionSettings`, sim-4)
**Dependencies:** `IEnumerable<ISimCombatantPolicy>` (for policy-id validation only), `IOptions<SimulationOptions>` (`ScenarioDirectory`, sim-3).
Registered as a singleton in `SimulationModule.AddSimulationModule`. Implemented (simulation-engine-core slice sim-2; save/list extended sim-3; `progression:` section extended sim-4).
...
Registered as a singleton in `SimulationModule.AddSimulationModule`. Implemented (simulation-engine-core slice sim-2; verdict-cell fallback extended sim-3).
...
**Purpose:** Runs a validated `ScenarioDefinition` end to end: kind dispatch to one of two executors (`Combat` → `CombatScenarioExecutor`; `ProgressionRate` → `ProgressionScenarioExecutor`, sim-4 — a two-way branch, rule-of-three; a third kind lands the executor-strategy seam, backlogged), one-time combatant pre-resolution, parallel fan-out across isolated `SandboxWorld` instances, deterministic index-ordered reduce, and verdict attachment via `ISimOutcomeEvaluator`. The `Combat` reduce produces `DistributionStats` (mean/median/p10/p90/min/max) for time-to-kill and per-side damage; the `ProgressionRate` reduce (sim-4) produces kills/XP-to-target distributions, per-milestone mean kill counts, and — when the scenario authors `ticksPerKill` — a `TicksToTarget` distribution, leaving the combat scalar fields at empty defaults. Publishes nothing (INV-5). Per-run seeds derive from `SimSeeds.DeriveRunSeed(scenarioSeed, runIndex)` — a stable SplitMix64-style mix, never `HashCode.Combine`/`Random.Shared` (INV-26) — so a fixed (scenario, seed) pair reproduces byte-identically regardless of `maxParallelism` or scenario kind. **sim-3 additive seams:** an optional `CancellationToken` (wired straight to the underlying `Parallel.For`'s `ParallelOptions.CancellationToken` — an already-canceled token throws before any run executes; cancellation mid-batch is checked cooperatively between per-iteration runs) and an optional `onRunCompleted` callback invoked once per completed iteration from worker threads (thread-safe, cheap, non-throwing contract; carries no data, so it cannot perturb seeds/scheduling/report content). Both default to inert — `SimulateRunMode` and every sim-2 call site are unchanged.
**Location:** `Core/Modules/Simulation/Systems/ISimulationRunner.cs` · `SimulationRunner.cs` · `CombatScenarioExecutor.cs` · `ProgressionScenarioExecutor.cs` (sim-4) · `SimSeeds.cs`
...
Registered as a singleton in `SimulationModule.AddSimulationModule`. Implemented (simulation-engine-core slice sim-2; cancellation + progress callback extended sim-3; `ProgressionRate` kind extended sim-4).

### ProgressionScenarioExecutor (Simulation module)
**Purpose:** Drives one `ProgressionRate` run to completion (sim-4): a kill-event loop that calls `IProgressionSystem.AwardCombatExperience(subjectId, victimId)` directly — the live award path minus the bus, mirroring how `CombatScenarioExecutor` mirrors `AbilityInvocationPipeline`. No combat rounds run per kill; the victim is never destroyed (one award models one kill of a fresh identical spawn). Records a milestone kill-count on every threshold crossing of the scenario's `TargetTrack`, terminating when `TargetImprovements` is reached or at the `MaxKillsPerRun` cap. Returns a `ProgressionRunRecord` (reached/not, kill count, per-milestone kill counts, final per-`CombatTracks` XP/improvement counts) — no bus (INV-5).
**Location:** `Core/Modules/Simulation/Systems/ProgressionScenarioExecutor.cs`
**Dependencies:** none (stateless; operates on the `SandboxWorld` passed to `ExecuteRun`, chance flows through the world's injected `IRandom` via `IProgressionSystem`).
Instantiated per run by `SimulationRunner`, not DI-registered (mirrors `CombatScenarioExecutor`). Implemented (sim-4).

### ISimOutcomeEvaluator / SimOutcomeEvaluator (Simulation module)
**Purpose:** Pure aggregation over caller-supplied counts — no run execution, no I/O. `Evaluate` compares combat win/draw counts against `IBalanceStandardsRegistry`'s expected-outcome tolerances — an equal-cell win-rate check when both sides share a (Tier, Band) cell, a one-band-higher win-rate-floor check when their global band indexes differ by exactly one, or a skipped-with-reason verdict otherwise (no cell, no decisive runs, or an undefined band-index gap); win rate excludes draws from the ratio (decisive-run share). **sim-4:** `EvaluateProgressionRate` produces two verdicts for every progression-rate report — `targetReached` (a real, standards-free pass = every run reached the target before the cap; reason carries the reached share) and `progressionRateExpectation` (always skipped, naming the not-yet-authored standards tolerance family — descriptive-first by design, since no designer has ever stated a progression-rate expectation). Reads the same tolerances the CLI, the sim-3 editor, and the promoted CI invariants all read (INV-19 — the math can never fork per surface).
...
Registered as a singleton in `SimulationModule.AddSimulationModule`. Implemented (simulation-engine-core slice sim-2; `EvaluateProgressionRate` extended sim-4).

### ISimReportWriter / SimReportWriter (Simulation module)
**Purpose:** Serializes a `SimulationReport` (schema version 1; additive fields never bump it) to a JSON artifact — a third durable class deliberately outside SQLite (INV-14 is for live entity state) and world YAML, the same posture as the `generate` run-mode's output. Atomic tmp→rename write into the configured `Simulation:ReportDirectory`; filename is `{yyyyMMdd-HHmmss}Z-{scenarioName}-{seed}.json`. Run history is the reports directory listing. Its `JsonSerializerOptions` are extracted into internal `SimReportJson` (sim-3) so the reader below shares the identical camelCase/enum convention rather than forking a second dialect. **sim-4:** the report carries an additive optional `ProgressionRateResult? ProgressionRate` — `null` on combat reports, populated (with the combat scalar fields at empty defaults) on progression-rate reports; a pre-sim-4 artifact with no `progressionRate` property still deserializes.
**Location:** `Core/Modules/Simulation/Systems/ISimReportWriter.cs` · `SimReportWriter.cs` · `SimReportJson.cs` · `Core/Modules/Simulation/SimulationOptions.cs`
**Dependencies:** `IOptions<SimulationOptions>`.
Registered as a singleton in `SimulationModule.AddSimulationModule`. Implemented (simulation-engine-core slice sim-2).

### ISimReportReader / SimReportReader (Simulation module)
**Purpose:** Read-side counterpart to `ISimReportWriter` (sim-3) — `List()` enumerates every file in `Simulation:ReportDirectory` newest-first, each row carrying just enough (`GeneratedAt`, `ScenarioName`) to render a history list without a second full parse; a file that fails to deserialize is flagged `Readable: false` with an `Error` message rather than aborting the whole listing. `Read(path)` fully deserializes one report. Shares `SimReportJson` with the writer, so a report reads back identically whether the CLI or the editor wrote it (one artifact class, two producers).
**Location:** `Core/Modules/Simulation/Systems/ISimReportReader.cs` · `SimReportReader.cs`
**Dependencies:** `IOptions<SimulationOptions>`.
Registered as a singleton in `SimulationModule.AddSimulationModule`. Implemented (sim-3).

### SimulationRunService (`Hedron.Web/Services/`)
**Purpose:** Singleton FIFO background-run registry for the Simulation page (sim-3; see [`../architecture/08-blazor.md`](../architecture/08-blazor.md) "Background tooling jobs"). `Enqueue` validates immediately (`ISimScenarioStore.Validate` — nothing is queued on a structural violation) and returns a run id; a single background drain loop runs one batch at a time via `ISimulationRunner.Run` (never concurrent — the engine already saturates cores per batch), tracks `Queued → Running → Completed/Failed/Canceled` plus a progress counter, and calls `ISimReportWriter.WriteAsync` on success. `Snapshot()` is the poll target; `Cancel(id)` marks a queued run canceled in place or signals a running run's per-run `CancellationToken` (a canceled batch writes no report). Bounded retention (~50) evicts the oldest *terminal* entries only — durable history lives in the report directory. No bus events, no `IHostedService` (INV-5/INV-10 — this is web-host UI state, not a live-world fact).
**Location:** `Hedron.Web/Services/SimulationRunService.cs`
**Dependencies:** `ISimulationRunner`, `ISimScenarioStore`, `ISimReportWriter`, `IClock` (status timestamps).
Registered as a singleton in `Hedron.Web/Program.cs` (web-host-only — not the shared `CompositionRoot`). Implemented (sim-3).

### BaselineSweep (`Hedron.Web/Services/`)
**Purpose:** Static scenario composer for the Standards page's "Re-run baseline sweep" affordance (sim-3) — scenario *selection*, not verdict math (the engine's validation/verdicts are reused untouched). `Compose(tunables)` builds one equal-cell scenario per (Tier, Band) cell and one adjacent-pair scenario per consecutive global-band-index pair, at fixed defaults (`seed 1234`, `iterations 200`, `maxTicksPerRun 100`, `policyId cooldown-first`) — deterministic names/seeds. Enqueued through `SimulationRunService`, the same run path the composer page uses.
**Location:** `Hedron.Web/Services/BaselineSweep.cs`
**Dependencies:** none (pure function of `PowerBudgetTunables`).
Static class, no DI registration. Implemented (sim-3).

### SimulationPrefill (`Hedron.Web/Services/`)
**Purpose:** Static composer for the "Simulate vs reference" entry points on `MobEditor`/`ItemEditor` (sim-3) — kept out of the razor pages so the prefill decision logic is independently testable. `ForMob` prefills the authored mob (as a `MobTemplate` source) against a `ReferenceBuild` of its authored (Tier, Band) cell, falling back to the computed cell (`IPowerBudgetSystem.Classify`/`IMobPowerProjectionSystem.Project`) when unbanded. `ForItem` composes an `Inline` source whose scores are the per-score sum of `IBalanceStandardsRegistry.ReferenceSnapshot(cell)` and `IItemPowerProjectionSystem.Project(template)` — "a baseline character of this cell wearing this item" — against a bare `ReferenceBuild` of the same cell; no weight/band/verdict math is re-derived, only summed. Reads the **last-saved** template via the catalog (matching every other cross-page editor link); prefill is navigation-only and never auto-runs. **sim-4 addition:** `TicksPerKillFrom(SimulationReport)` — a pure static prefill for the progression-rate composer's `ticksPerKill` field, returning a decisive `Combat` report's `TicksToKill.Mean` or `null` (wrong kind, or zero decisive runs); the engine never reads report files as input, so this stays an editor-side convenience only. `ProgressionSettingsForm` (same directory) is the kind + `ProgressionSettings` ↔ form composer the Simulation page's progression fields round-trip through.
**Location:** `Hedron.Web/Services/SimulationPrefill.cs` · `ProgressionSettingsForm.cs` (sim-4)
**Dependencies:** none (pure functions taking `IPowerBudgetSystem`/`IMobPowerProjectionSystem`/`IItemPowerProjectionSystem`/`IBalanceStandardsRegistry` as parameters).
Static class, no DI registration. Implemented (sim-3).
