...
| `ItemDataComponent` | `Name`, `Description`, `Keywords` (`List<string>`), `ItemType`, `WornSlots` (`List<WornSlot>?` — null/empty = not wearable), `StatBonuses` (`List<EquipmentStatBonus>` — authored worn-gear stat contributions, derived on read as `WhileEquipped` StatModifiers by `EquipmentEffectContributor`; empty = none) | Item entity; read by `BroadcastSystem` (room description), `ItemSystem` / `ItemBuilderSystem` / `EquipmentSystem`, and `EquipmentEffectContributor` (stat fold) | yes |
