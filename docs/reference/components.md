...
| `ItemDataComponent` | `Name`, `Description`, `Keywords` (`List<string>`), `ItemType`, `WornSlots` (`List<WornSlot>?` — null/empty = not wearable), `StatBonuses` (`List<EquipmentStatBonus>` — authored worn-gear stat contributions, derived on read as `WhileEquipped` StatModifiers by `EquipmentEffectContributor`; empty = none), `Value` (`long` — intrinsic base value in base-unit Coin; `0` = valueless/non-saleable; prices are derived from this at read time by consumers, never stored), `Tier: int` (0–6, 0 = unbanded/base; the mechanical Ascension tag mirroring `MobDataComponent.Tier`, prog-3b) + `Band: int` (0–3, 0 = unbanded; purely descriptive low/mid/high subdivision within `Tier`, prog-3b — used only for the authored-vs-computed comparison in the `power` inspector / Blazor readout, grants no power) | Item entity; read by `BroadcastSystem` (room description), `ItemSystem` / `ItemBuilderSystem` / `EquipmentSystem`, and `EquipmentEffectContributor` (stat fold) | yes (component-level), but `Tier`/`Band` never reach a snapshot in practice for world-spawn items — durable form is the `ItemTemplate` YAML, re-applied on spawn |
...
| `MobDataComponent` | `Name`, `Description`, `Keywords` (`List<string>`), `MobType`, `Tier: int` (0–6, 0 = unbanded/base; mechanical Ascension tag, prog-2/3b) + `Band: int` (0–3, 0 = unbanded; purely descriptive low/mid/high subdivision within `Tier`, prog-3b) + `XpScale: double` (non-negative, `1.0` = default, `0` = this mob's kills award nothing; the per-mob granular progression knob, prog-6 — read inside `ProgressionSystem.AwardCombatExperience` so the live and simulated kill paths share one read site) — cross-cutting so combat, AI, and dialogue modules can read mob names without a domain dependency | Mob entities; read by `BroadcastSystem` (room description) | yes (component-level), but `Tier`/`Band` never reach a snapshot in practice — mobs are world content (no `PersistentEntity`); durable form is the `MobTemplate` YAML, re-applied on spawn |
...
| Progression | `ProgressionComponent` | `Xp: Dictionary<ProgressionTrack, int>` (cumulative, never decremented), `Improvements: Dictionary<ProgressionTrack, int>` (thresholds crossed per track — the power-step count for a score track, the display-only rank for an ability track). A `ProgressionTrack` is **either** a `ScoreId` **or** an ability id — one vocabulary over one engine, no parallel key type and no second XP store (prog-6). The derived power step is **not** stored here — pulled on read by `ProgressionEffectContributor` (INV-24). Attached lazily on an entity's first `AwardExperience` call — always a persistent (player) entity. Keys serialized by `ProgressionTrackJsonConverter` (attached via `[JsonConverter]` on the struct, overriding `WriteAsPropertyName`/`ReadAsPropertyName` because the type is a dictionary key): a score track renders as the bare enum name, byte-identical to the pre-widening form, so pre-slice snapshots load unchanged and **no migration is needed**; ability tracks take the reserved `ability:` prefix. Located in `Core/Modules/Progression/Components/`. | yes |
| Preferences | `PlayerConfigurationComponent` | `Preferences: Dictionary<PreferenceId, bool>` — **sparse**: only preferences the player explicitly set are stored, and an absent key resolves to the `PreferenceRegistry` default, so changing a shipped default reaches every player who never touched it and adding a preference needs no migration. Attached on first `IPreferenceSystem.Set` — never by a read. Player characters only (they already carry `PersistentEntity`, INV-23). The prompt-template field the planned component also names folds in here when the prompt slice needs it, rather than as a second component. Located in `Core/Modules/Preferences/Components/`. | yes |
