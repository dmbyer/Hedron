# Systems Reference — Planned / Idealized

> **Design intent, not a catalog of what exists.** Every system below is **not yet built**. The interfaces are idealized sketches written against the target architecture; they predate current component shapes and will be re-designed against reality when the slice that needs them lands. **Do not assume any of these exist or that their signatures are final.** For systems that actually exist, see [`systems.md`](systems.md).
...
```
The heartbeat/scheduler is built on `RegisterTimer` (many timers at different cadences, not one global pulse) — see [`../architecture/01-layers.md`](../architecture/01-layers.md#heartbeat--scheduled-tick--forward-design-constraints). Expected to land with the mob-wandering slice.
...
### NotificationSystem
**Purpose:** Targeted single-/multi-entity message delivery (the per-entity counterpart to the implemented room/all fan-out in `BroadcastSystem`).
**Dependencies:** `IVisibilitySystem`, `ILocationSystem`, `IOutputWriterFactory`.
```csharp
public interface INotificationSystem
{
    Task Send(Entity recipient, IOutputMessage message);
    Task SendToMany(IEnumerable<Entity> recipients, IOutputMessage message);
}
```
> **Reconciled with the output framework.** Earlier drafts of this interface passed raw `string` messages and a `MessageTemplate` overload — both violate INV-11 (no raw transport strings; output is always a typed `IOutputMessage`). If this system is ever built it must take `IOutputMessage` and render via `IOutputWriterFactory`, exactly like `BroadcastSystem`. Much of its intended role (room broadcast, system-wide) is **already covered** by the implemented `BroadcastSystem`; build this only if a genuine per-recipient, visibility-filtered send is needed that broadcast's room/audience-filter model can't express.

---

## Target dependency graph (planned)

The graph below is the intended shape once the planned domain/core systems exist. It must always be a DAG. If you find yourself wanting an upward arrow, see [`../architecture/04-pitfalls.md#circular-dependencies`](../architecture/04-pitfalls.md#circular-dependencies).
