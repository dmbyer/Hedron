...
7. **Integrity sweep.** The Integrity page calls `IContentReferenceIndex.SweepBroken()` and tabulates every broken link (source kind, id, field/direction, missing target) with edit links — the offline "assurance" surface before reload.
