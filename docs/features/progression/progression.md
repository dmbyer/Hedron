...
| [`power-budget-system.md`](power-budget-system.md) | `IPowerBudgetSystem`/`PowerBudgetSystem` (core-tier oracle — `Estimate`/`Classify`→`PowerBand`/`TargetRange`/`BandAnchor`), `PowerBudgetConstants` (weight table, band span, `BandsPerTier`, mirrored reference-build + tier constants), `IItemPowerProjectionSystem`/`IMobPowerProjectionSystem` (shared projection seams), `IBalanceAuditSystem` (bulk drift sweep); `power`/`powerband` inspectors; item `Tier`/`Band` content tags |
