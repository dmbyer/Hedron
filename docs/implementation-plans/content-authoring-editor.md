...
- **Docs.** `docs/architecture/flows/README.md` gains Flow 30 (offline content edit → save → apply). `docs/reference/systems.md` gains `IContentDefinitionCatalog` (Authoring) and `IContentValidator` (World). `docs/architecture/09-documentation.md` / module map notes the new `Hedron.Web` host. `docs/reference/components.md` is unchanged (no new components).
...
> Durable seam rationale lives in [`content-tooling-platform.md`](content-tooling-platform.md) (Design notes + Architecture brief). This section records only the T2-specific seam resolutions (INV-27 — no restatement).
