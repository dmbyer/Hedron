...
2. Room YAML round-trips `spawnRules` losslessly (writer-fidelity fix — regression-locked).
...
9. Two rooms in the same area with identical non-null `X/Y/Z` produce a registry-validation **warning** (never an error); rooms in different areas or on different Z-layers do not.
...
| `RoomTemplate` + `RoomTemplateDeserializer` + `RoomContentWriter` | **extended** | Optional `X/Y/Z` (`int?`) fields, camelCase YAML `x`/`y`/`z`, omitted-when-null; writer DTO also gains `spawnRules` (fidelity fix). `Apply` unchanged. |
...
| `IContentValidator` (`ContentValidator`) | **extended** | Registry mode gains one warn-not-error rule: coordinate collision within an area. |
...
- **Scope:** `RoomTemplate.X/Y/Z` (`int?`, settable); `RoomTemplateDeserializer` reads `x`/`y`/`z`; `RoomContentWriter` DTO gains `X`/`Y`/`Z` (nullable, omitted when null) **and `SpawnRules`** (fidelity fix); quick audit of `Area/Item/MobContentWriter` for equivalent round-trip losses (fix in-scope if found, same class of bug); `DirectionExtensions.Offset`/`FromOffset`; `RoomCoordinateCollisions` helper; `ContentValidator` registry-mode collision warning; `IContentDefinitionCatalog.RemoveRoomExitAsync` + implementation.
- **Files:** `Core/Modules/World/Templates/RoomTemplate.cs`, `RoomTemplateDeserializer.cs`; `Core/Modules/World/Systems/RoomContentWriter.cs`, `IContentValidator.cs`/`ContentValidator.cs`, `RoomCoordinateCollisions.cs` (new); `Core/Direction.cs`; `Core/Modules/Authoring/Systems/IContentDefinitionCatalog.cs`, `ContentDefinitionCatalog.cs`; tests under `Hedron.Tests/World/` + `Hedron.Tests/Authoring/`.
- **Dependencies:** none.
- **Out of scope:** any runtime component or `Apply` change; exit↔coordinate enforcement; layout math; UI.
- **Exit criterion:** `dotnet test` green with new tests: room YAML round-trip (coords + spawnRules + legacy-file nulls), offset totality/inverse, collision-warning rule, `RemoveRoomExitAsync` policy matrix.
...
|---|---|---|
| Content templates / YAML round-trip | **Gap exposed → closed in-slice (WP1)** | `RoomContentWriter` drops `spawnRules` today — pre-existing silent data loss on every editor re-save, amplified by the grid's neighbor rewrites. Fixed with regression round-trip test; sibling writers audited for the same class. |
...
| Validation framework | **Adequate** | One registry-mode warn-not-error rule; detection logic declared once (`RoomCoordinateCollisions`) and shared with the grid via the layout proposal. |
...
| System unit (T1) | `ContentValidatorTests` (addition) | Same-area same-cell duplicate → warning, report still valid; different area / different Z → no warning (→ 9). |
| Persistence round-trip (T4, YAML) | `RoomContentWriter` ↔ `RoomTemplateDeserializer` | `X/Y/Z` survive write→read; nulls omitted from file text and read back null; legacy file loads with nulls; **`spawnRules` survive write→read** (regression) (→ 1, 2). |
| System unit (T1) | `RoomTemplate.Apply` (existing test file touch) | No coordinate-bearing runtime component attached (→ 3). |

**Skipped, with rubric reason ([`07-testing.md`](../architecture/07-testing.md)):** `AreaGridEditor.razor`, `RoomExitsEditor.razor`, `RoomBasicsFields.razor`, `RoomEditor` refit — presentation skip-tier; every decision they invoke (validation, bidirectional policy, layout, collision) is covered at T1/T4. No handler/flow/architecture-guard additions — no bus, no live world, no new guard-worthy convention. Postconditions 4–5 are compositions of already-tested catalog verbs exercised by the WP3 manual checklist. **Testability gaps: none** — the layout system is deterministic by construction (no RNG, no clock; INV-26 moot) and the catalog test harness (temp content directory) already exists.
...
- **INV-20** — 08-blazor.md JS-presentation-only note deferred with the first JS asset (none in v1).
...
- **Confirm decision 6** (Apply-layout bulk write UX) and **decision 8** (Blazor-only v1, click-to-connect instead of drag) — both are planner recommendations within seed-approved bounds; neither changes the architecture if reversed, but 8 changes the WP3 interaction spec.
- **Pre-existing `spawnRules` write-loss fix (WP1)** is scoped into this slice as a gap-exposed finding; if the user prefers it as a separate hotfix ahead of the slice, WP1 splits cleanly.
