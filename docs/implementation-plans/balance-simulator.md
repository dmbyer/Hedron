...
| **sim-3** | **Editor integration — ✅ done** | Blazor **Simulation page**: compose/launch scenarios (background execution in `Hedron.Web`), live status, report viewing over the run-history artifacts; **"simulate this" entry points** on `MobEditor`/`ItemEditor` (authored content vs the reference build of its authored cell); standards page gains "re-run baseline sweep" affordances. Durable design in [`../features/simulation/simulation.md`](../features/simulation/simulation.md); as-built history in [`../roadmap/completed/simulation-editor-integration.md`](../roadmap/completed/simulation-editor-integration.md). | sim-2 |
| **sim-4** | **Progression-rate scenarios — ✅ done** | Second scenario **kind** on the same runner: analytical kill-event sweeps over `IProgressionSystem.AwardCombatExperience`, an authored `ticksPerKill` bridge from combat-report time-to-kill, targeting improvements-only (time-to-tier deferred, no mechanical gate exists yet); reports render in the same editor page (kind-generic report shape proven out). Durable design in [`../features/simulation/simulation.md`](../features/simulation/simulation.md) / [`simulation-engine.md`](../features/simulation/simulation-engine.md); as-built history in [`../roadmap/completed/progression-rate-scenarios.md`](../roadmap/completed/progression-rate-scenarios.md). | sim-2 (sim-3 for UI) |
...
**Next:** sim-1, sim-2, sim-3, and sim-4 shipped — see [`../roadmap/completed/balance-standards-registry.md`](../roadmap/completed/balance-standards-registry.md) / [`../roadmap/completed/simulation-engine-core.md`](../roadmap/completed/simulation-engine-core.md) / [`../roadmap/completed/simulation-editor-integration.md`](../roadmap/completed/simulation-editor-integration.md) / [`../roadmap/completed/progression-rate-scenarios.md`](../roadmap/completed/progression-rate-scenarios.md) and [`../features/progression/power-budget-system.md`](../features/progression/power-budget-system.md) / [`../features/simulation/simulation.md`](../features/simulation/simulation.md). The remaining sub-slice, `sim-5` (template/instance conformance tooling), gets its own plan framed against this seed via `/new-plan`.
