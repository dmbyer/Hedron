...
- **On ship:** author `architecture/subsystems/abilities.md` (the living design of the ability system — kinds, activation pipeline, cooldown model) and trim this doc to requirements + the durable behavior spec (docs lifecycle, R7; INV-28). The `IEffectContributor` seam is **already canon** (INV-24; [`effects.md`](../architecture/effects.md#the-contributor-seam); [`01-layers.md`](../architecture/01-layers.md#the-three-composition-shapes); the `add-core-system` skill) — WP-1 implements the first contributor; WP-3's catalog sweep updates `systems.md`/`effects.md`.
