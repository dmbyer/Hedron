...
| **3b** | **Power model revision — Tier × Band + calibration + audit tooling** | Two-axis `Classify` output (`Tier 0–6` × `Band 1–3` within each tier — a D&D-Challenge-Rating-style model, not a finer leveling system); a `(Tier, Band) → target power range` inverse query for forward design + future procedural generation; `TierBand` becomes a `Tier`+`Band` pair on `MobDataComponent`/`ItemDataComponent`/templates/YAML/`setmob`/`setitem`/both Blazor editors; band-count-tolerance drift detection (soft — an upgraded editor mismatch flag, plus a bulk audit report extending the existing Integrity sweep, never a build-blocking gate) that doubles as free "how much content exists at power level X" reporting; recalibration of `PowerBudgetConstants` against real design targets (the shipped placeholder numbers were never validated — a spot-check read a near-blank character as Tier 4, confirming the scale needs real headroom, not guessed constants); and a written-down extensibility principle for future power inputs (skills, spells, effect grants, speed, defensive ratings) so the oracle stays snapshot-only and never gains a domain import — pulled forward from slice 5's INV-20 scope because it's a direct dependency of this revision, not because the whole doc-layer moved up. Seeded by `/advise` into [`power-model-revision.md`](power-model-revision.md) (2026-07-07) — Open questions 7–12 resolved, plus the calibration-scope (oracle-only) and field-migration (clean break) forks; next is `/new-plan`. | 3 |
| **4** | **Simulation harness** | `Hedron.Sim` project (parameter-sweep runner, statistical reporting, combatant-policy seam), promoted CI regression invariants | 3b (uses the revised power oracle, not the shipped one) |
...
7. **Band is a second axis inside Tier, not a synonym for it** (slice 3b) — ✅ **RESOLVED: Tier (0–6, character-wide, Ascension-gated) stays the coarse "chunk of levels" axis; Band (1–3, within each tier) is a finer subdivision — low/mid/high — "enough progression to feel meaningful, but not introducing an entire leveling system."** This is a genuine D&D-Challenge-Rating-style model: given a mob/item's stats, classify where it falls (Tier, Band); given a target (Tier, Band), get a stat range to design toward. The shipped slice-3 `Classify` returning one 0–6 number conflated these two axes — it needs to become `(Tier, Band)`, with ~21 anchors (3 sub-anchors per tier gap) instead of 7.
8. **Band tolerance metric** (slice 3b) — ✅ **RESOLVED: band-count, not raw-power-percentage.** "More than N bands off" is simpler and stays self-consistent with the Tier×Band vocabulary the rest of the design is in; a percentage-of-power metric would need its own separate tunable constant with no shared meaning.
9. **What does authoring a band actually unlock, and is it worth keeping as data** (slice 3b) — ✅ **RESOLVED: keep it persisted (Option A), but make it enforceable, not decorative.** As shipped, the authored `TierBand` tag had exactly one consumer (a UI diff-check) and gated nothing — a designer could type any number with no feedback loop. Kept because a persisted tag turns "how much content exists at power level X" and "what drifted after a balance retune" into a cheap grouped scan instead of a full-catalog recompute every time someone asks. Two soft-enforcement layers, **neither a hard blocker**: (a) the existing per-item editor mismatch flag, upgraded from exact-match to band-count tolerance; (b) a bulk audit report extending the existing Integrity sweep page (plus likely a headless/admin equivalent mirroring `powerband`) that lists all content past tolerance and buckets counts by `(Tier, Band)` — this second layer is also the free answer to the reporting/auditing need. A CI hard-fail gate was explicitly considered and rejected — variance within a band is expected, not a bug.
10. **Calibration of the actual numbers** (slice 3b) — ✅ **RESOLVED as a disposition (2026-07-07): oracle estimation only.** 3b recalibrates `PowerBudgetConstants` for real headroom + deliberate 3-band spacing but leaves `AscensionConstants` (real gameplay tier power) untouched — the mirror stays in sync; concrete numbers are hand-derived now (documented still-heuristic) and sim-validated in prog-4, which owns any gameplay-power retune. The remaining detail below is the diagnostic that informed this. The shipped `PowerBudgetConstants.Weights`/`ReferenceBaseScores`/`BandSpan` were placeholder values, documented in-code as "heuristic and tunable — not a precise truth" but never validated against real gameplay feel. A spot-check (a near-blank character reading `power` as Tier 4 on a scale that "doesn't even go up to 1000 for tier 6") confirmed the numbers need real headroom and deliberate spacing — though note this specific instance may also reflect that test character's `AscensionComponent.Tier` already being non-zero from earlier manual testing (Ascension tier is admin-set today, independent of progression XP) rather than a pure math bug; check via `score` before assuming the formula itself is broken. Either way, real target numbers (not guessed ones) are `3b`'s job, ideally cross-checked once `Hedron.Sim` (slice 4) can validate them against actual combat outcomes.
11. **Power-model extensibility for future inputs** (slice 3b) — ✅ **RESOLVED: the oracle stays snapshot-only; it never gains a domain import to learn about abilities/effects/speed/defense directly** (the same INV-2 boundary slice 3's code-review gate already caught once with `AscensionConstants`). Future power sources fold in one of two ways: (a) a genuinely stat-like quantity (Speed, Evasion, …) becomes a new `ScoreId` vocabulary member callers can weight; or (b) a richer source (an ability's burst damage, an effect's grant) computes its **own** estimated power contribution and a caller sums it into the snapshot before calling `Estimate` — mirroring the existing `IEffectContributor` port precedent, never taught to the oracle itself. This needs to be written down as a named principle (a design-doc note, not just prose in one feature file) so it survives future slices without being silently violated again.
12. **Pulling prog-5 scope forward** (slice 3b) — ✅ **RESOLVED: partially.** Only the INV-20 doc/skill piece that is a *direct dependency* of item 11 above (teaching `add-domain-system`/`add-core-system`/the advisor to ask "does this affect power, and how does its contribution enter the snapshot") is pulled into slice `3b`. The balance catalog (`design/balance.md`) and the `balance-tuning`/`run-simulation` skills stay at slice 5, since `run-simulation` has no meaning before `Hedron.Sim` (slice 4) exists.
