...
| **4** | **Simulation harness — expanded into its own sub-program — ✅ done** | Expanded (advisor intake 2026-07-13) from a dedicated `Hedron.Sim` project into a Core module (`Core/Modules/Simulation/`) drivable by the Blazor editor, plus a data-driven balance-standards registry, editor integration, progression-rate scenarios, and template conformance tooling — five sub-slices (`sim-1`–`sim-5`), all shipped. The combatant-policy seam and promoted-CI-invariant decisions carried over unchanged. Durable design now lives in [`../features/progression/power-budget-system.md`](../features/progression/power-budget-system.md) and [`../features/simulation/simulation.md`](../features/simulation/simulation.md); as-built history in [`../roadmap/completed/balance-standards-registry.md`](../roadmap/completed/balance-standards-registry.md), [`../roadmap/completed/simulation-engine-core.md`](../roadmap/completed/simulation-engine-core.md), [`../roadmap/completed/simulation-editor-integration.md`](../roadmap/completed/simulation-editor-integration.md), [`../roadmap/completed/progression-rate-scenarios.md`](../roadmap/completed/progression-rate-scenarios.md), and [`../roadmap/completed/conformance-tooling.md`](../roadmap/completed/conformance-tooling.md). | 3b (done) |
...
### Simulation engine — deterministic batch simulation, cheap *because of* the architecture (shipped as sim-2; superseded the `Hedron.Sim`-project framing below)

**As shipped (sim-2, see [`../features/progression/power-budget-system.md`](../features/progression/power-budget-system.md) and [`../roadmap/completed/simulation-engine-core.md`](../roadmap/completed/simulation-engine-core.md)):** a **Core module** (`Core/Modules/Simulation/`), registered via `CompositionRoot` so `Hedron.Web` can resolve it too — not a dedicated project, per the [`balance-simulator`](balance-simulator.md) program seed's revision. Original framing kept below for history: systems are already near-pure functions driven by injected `IRandom` and synthetic ticks — the same property that makes Tier-1 tests trivial — so a batch simulator is "run the [Tier-3 flow harness](../architecture/07-testing.md) N× with a parameter sweep and assert on the *distribution*," mirroring (not reusing) `EntityBuilder`/`FakeRandom`/synthetic ticks via its own hand-built sandbox factory (no event bus at all — even a `RecordingEventBus` would be a live-world dependency the engine deliberately avoids). It sweeps build × mob × tier, emits statistical reports (win rate, time-to-kill, damage distributions), and uses the sim-1 balance-standards registry as its expected-vs-actual oracle. A **thin subset** of derived invariants (e.g. "a Tier-N baseline PC beats a Tier-N baseline mob within band X") is promoted into `Hedron.Tests` as CI regression gates — keeping heavy sweeps out of the test suite while still catching balance regressions.
...
| Simulation engine (runner, policies, reporting) | Domain (tooling-tier) | `Core/Modules/Simulation/` — shipped sim-2, not a separate project |
...
| Fork 3 — Sim home | ~~Dedicated `Hedron.Sim` project~~ **superseded — `Core/Modules/Simulation/` (shipped sim-2, see `balance-simulator.md`)** + thin promoted CI invariants (no test-suite bloat; enables distribution analysis). |
