...
4. Event bus dispatches to all subscribed handlers in priority order:
   ...
   - `CombatTickHandler` (priority 20) runs combat round processing (see [Flow 18](flow-18-combat-round-pulse.md)). Writes `CombatMessage`s (category=`Combat`, policy=`Batched`) to affected session buffers.
