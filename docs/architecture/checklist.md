...
**INV-31 — Declared concurrency posture at every cross-thread surface.** The runtime is multi-threaded at its edges (per-session telnet read-loop threads, `BackgroundService` initiators like the heartbeat and flush timer, Blazor circuit threads, sim worker threads). State reachable from more than one of those threads must be either **guarded** by its owning infrastructure type (precedents: `EventBus`, `SessionOutputBuffer`/`SessionBufferRegistry`, `SessionManager`, `TelnetSession`, `TemplateRegistry`) or **confined** to a single thread/world (precedent: each sim run's sandbox `EntityService`). A slice adding a background initiator, a new session-thread entry point, a web-host surface that calls engine systems, or shared mutable singleton state names its posture (guard vs. confine) in the plan. **Acknowledged debt:** live ECS component storage (`ComponentRepository`) is unguarded under concurrent session/heartbeat access — the world-state threading model decision is tracked in [`../roadmap/backlog.md`](../roadmap/backlog.md); until it lands, new work must not widen that exposure (no new thread/timer mutating live world components outside the existing session-command and heartbeat-handler paths). Explanation: [04-pitfalls.md](04-pitfalls.md#threading-model--cross-thread-state).
- *Spec:* does the plan introduce a new thread, timer, background service, or web-host call path into engine state without naming its concurrency posture? Does it direct a new thread to mutate live world components?
- *Code:* new shared mutable static/singleton state with no guard; a new `Task`/`Timer`/`BackgroundService` reaching into `EntityService`/live components outside the established paths.
