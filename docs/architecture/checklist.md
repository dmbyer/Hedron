...
>
> Each invariant has a stable ID (`INV-n`). Cite the ID in reviews (e.g. "INV-7 violated at `Foo.cs:42`"). "How to check" distinguishes **spec mode** (reviewing an implementation plan before implementation) from **code mode** (reviewing a diff) only where the technique differs.
