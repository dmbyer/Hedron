...
**INV-21 — Blueprint definition and blueprint instance are separate.** A blueprint template (content file or in-memory registration) is the durable definition of what to spawn; a blueprint instance is the live entity created from that template, tracked via `BlueprintComponent.BlueprintId`. When a player interaction converts a blueprint instance to independent player-owned state (e.g. picking up a spawned item), `BlueprintComponent` must be cleared on the entity before saving — this frees the blueprint slot so the next restart can spawn a fresh instance in the spawn room. An admin mutation (`setitem`, `setroom`, etc.) updates the template definition and the live entity; decoupled player-owned instances are not retroactively updated.
- *Spec:* does the use case describe player-touch transitions? Does it commit to clearing `BlueprintComponent` on the entity, and to keeping the template file in sync with any admin mutation?
- *Code:* does any player-interaction save path retain `BlueprintComponent.BlueprintId` on a player-owned entity? Does any admin mutation update the entity without also updating the template definition?
