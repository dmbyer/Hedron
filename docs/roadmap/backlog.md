...
**Update (2026-08).** The client-tier analysis in [`../design/client-tier.md`](../design/client-tier.md) sharpened three things about this item. (1) **The seam is thinner than [`../architecture/08-blazor.md`](../architecture/08-blazor.md) claimed:** `TransportKey` is real, but `IOutputFormatter.Format` returns `string`, so it buys another *text* transport — a rich client needs a structured formatter emitting the typed output messages as JSON, which is additional work either way. (2) **`LoginFlow` is nearly ready:** its only `StreamReader` coupling is a single `ReadLineAsync` call, so an `ILineReader` extraction (~20 LOC) opens the whole login state machine to any transport. (3) **This item is blocked on the world-state threading decision above** — a hub invocation mutates the live world from a request thread, which is exactly what INV-31's "don't widen the exposure" clause forbids. That blocker is framework-independent: it applies equally to a Blazor and a React client, which is why the client-tier gate does not depend on resolving it first.

### 🔵 "Apply to live" cannot reach a running game server (found by the client-tier analysis, 2026-08)

[`Hedron.Web/Components/Pages/Apply.razor`](../../Hedron.Web/Components/Pages/Apply.razor) documents the limitation in its own callout: the reload runs inside the authoring host's process — a preview/validation world with no heartbeat — so applying authored content to the *actual* running game remains a manual server-side reload or restart. This is the structural cost of the in-process-DI shortcut: because the editor is in-process with *an* engine, it is out-of-process with *the* engine, and there is no transport to point at the game host.

Not a Phase 5 blocker (the authoring workflow tolerates a restart), but it is the editor's headline action failing to do what its name says, and it should be tracked as debt rather than left as a documented limitation. The fix needs a reload trigger exposed on the telnet game host — a genuinely new host shape (the game host gains a web surface) carrying its own auth question, so it warrants `/advise` framing rather than being folded into an authoring slice. The narrow JSON surface in [`completed/authoring-api-surface.md`](completed/authoring-api-surface.md) (shipped) is the client-side half; this item is the server-side half.
...
**Update (blueprint-id-editing).** The blueprint-id rename cascade (`RenameAsync`, [`completed/blueprint-id-editing.md`](completed/blueprint-id-editing.md)) shares this **exact** best-effort posture — it cascade-*rewrites* `oldId → newId` across referrers (the same traversal delete uses, generalized so clear = rewrite-to-empty), writes the new-id file, then deletes the old, none of it transactional. Covered by the same transactional-cascade fix; no separate work.

### 🔵 Cross-process content-cache coherence (acknowledged debt from authoring-editor-repair)

`ContentDefinitionCatalog` now serves reads from an in-memory index that **every catalog mutator invalidates**. That makes the editor coherent with its own writes, but the catalog is *not* the only writer of content YAML: the `generate` CLI runs in a separate process against the same directory ([Flow 29](../architecture/flows/flow-29-bulk-content-generation.md) leg A), and the game host's `mk*`/`set*`/`dig` verbs write through the `I*ContentWriter` family directly. Neither invalidates a running editor host's index, so a long-lived editor can serve a stale listing after an out-of-process write.

Mitigation shipped in the slice: the content browser's **Refresh from disk** action calls `IContentDefinitionCatalog.Invalidate()`. The real fix is a `FileSystemWatcher` on the content directory (debounced, invalidating on any change), which carries its own INV-31 posture — the watcher fires on a threadpool thread and would become a second writer of the index. Deliberately out of scope while authoring is single-author and offline; lands when either the editor host is long-running alongside an active game host, or an author is bitten by the staleness in practice.

### 🔵 Editor form-shell component (acknowledged debt from authoring-editor-repair)

Keyboard-first authoring (autofocus, Enter-to-save, Esc-to-cancel) is implemented **ad hoc in each of the four content editors** — `AreaEditor`, `RoomEditor`, `ItemEditor`, `MobEditor` each carry their own `OnFormKeyDown` + `_focusPending` pair, as does the "Save & create next" button wiring. That is four copies of the same shell — **past INV-19's "repeating a hand-rolled pattern ≥3 times" trigger**, answered here as acknowledged debt rather than extracted, because the client-tier gate ([`../design/client-tier.md`](../design/client-tier.md)) could moot a Blazor-specific form shell entirely. (An earlier draft of this entry claimed the bar was "≥3 *distinct* patterns" and that four instances of one pattern therefore did not reach it. That is wrong — the checklist wins, and distinctness is not part of the rule.) **Extract a shared form-shell component at a fifth editor, or as soon as the gate resolves toward Blazor** (`Hedron.Web/Components/Shared/`) — owning focus, the keyboard contract, and the save/save-next/cancel action row, with each editor supplying its fields. A React port would build its own shell, which is the whole reason to wait.

### 🔵 Live-world / persistent blueprint-id rename (deferred from blueprint-id-editing)

The blueprint-id-editing slice ([`completed/blueprint-id-editing.md`](completed/blueprint-id-editing.md)) ships **YAML-only** rename in the offline editor: it rewrites every YAML reference to the old id, and the live world adopts the new ids on the next `reload`. References that live **outside** YAML are deliberately **warned, not rewritten** — `LocationComponent.RoomBlueprintId` (persistent player/item state, SQLite) and `World:StartingRoomBlueprintId` (`appsettings.json`). A player saved in a renamed room falls back to the starting room via `CharacterHydrationHandler`'s existing unresolvable-`RoomBlueprintId` recovery; the config must be hand-updated. This matches the editor's YAML-only posture (INV-22/23) and delete's existing behavior.
...
**Update (authoring-editor-repair).** The trigger was examined and **not** fired. That slice moved the Integrity page's cross-reference + balance sweeps off the circuit thread into `ContentIntegritySweepService` (`Hedron.Web/Services/`), but that job is **progress-only — no queue, no cancellation** — so it does not match the shape the trigger is about. The recorded candidate remains sim-5's bulk conformance apply (`PreviewAllFlagged`/`ApplyAllFlagged`), which **still runs blocking on the circuit thread in that same page** and was deliberately left there. That is the job that fires this trigger when it moves.
...
**Update (2026-08).** The Blazor editor shipped and is in use, and its *quality* is now the open question rather than its existence: see [`../design/client-tier.md`](../design/client-tier.md) for the Blazor-vs-React decision and its Phase 5 → 6 gate, and [`completed/authoring-editor-repair.md`](completed/authoring-editor-repair.md) for the repairs that landed inside Phase 5. The convention this item established — authoring logic in systems, the surface a thin caller — held on the engine side, which is what makes the front end cheap to change either way; the exceptions where logic did leak into components were enumerated and extracted in [`completed/authoring-api-surface.md`](completed/authoring-api-surface.md).
...
**Update (2026-08) — partially promoted and shipped, deliberately narrow.** The `authoring-api-surface` slice landed a scoped slice of this item. Corrections to the framing above, made in that PR:

- **The justifying consumer is one, not two.** "CI-testable authoring operations" is **withdrawn as a justification** — it is circular (endpoints existing so endpoints can be tested), and the coverage hole it pointed at is closed more cheaply at the system tier. It remains a real side benefit. The single justifying consumer is the bakeoff page the client-tier gate needs ([`../design/client-tier.md`](../design/client-tier.md)), which cannot fire without it.
- **"Auth stays deferred only because the bind stays loopback" was wrong.** Loopback is a *weaker* control for an unauthenticated minimal-API write endpoint than it is for the Blazor page: `UseAntiforgery()` does not cover it, any local process can reach it, and any page in the author's browser can reach it cross-origin (localhost CSRF, DNS rebinding). The deferral is still legitimate on different grounds — INV-19 enumerates *player-facing* surfaces and this is admin/authoring — and it shipped with a cheap in-slice mitigation (loopback `Host`, same-origin `Origin`, JSON `Content-Type`) plus a standing rule that **no CORS policy may be registered on this host**, guarded by tests. See [`../architecture/08-blazor.md`](../architecture/08-blazor.md#the-authoring-http-surface-api).
- **"It carries its own auth + DTO surface" — half done.** The DTO surface landed (`IContentDefinitionMapper<TDto>` in `Core/Modules/Authoring/Contracts/`, plus an OpenAPI drift gate in CI). Auth did not; that half of the stipulation is now the live remainder of this item.

**Still deferred here:** the full ~30-endpoint surface across every authoring system, write endpoints for kinds other than mob, any endpoint on the telnet game host, an apply/reload endpoint on the authoring host (it would be an Initiator — a different posture), and **authentication**, which is the hard prerequisite before any non-loopback bind.
