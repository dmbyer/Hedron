...
**Update (2026-08) — partially promoted, deliberately narrow.** Two concrete consumers now exist, so a **scoped** slice of this item is seeded as [`../implementation-plans/authoring-api-surface.md`](../implementation-plans/authoring-api-surface.md): CI-testable authoring operations (the endpoints are integration-testable under `dotnet test`, unlike the presentation-skip-tier components), and the bakeoff page the client-tier gate needs ([`../design/client-tier.md`](../design/client-tier.md)). The "only if a consumer is real" constraint above is respected by scoping to those two — **not** the full ~30-endpoint surface, which stays deferred here. The thin-transport-adapter and no-re-implementation rules are carried into the plan unchanged; auth stays deferred only because the bind stays loopback.
