...
**Update (2026-07).** The [`progression-and-balance`](../implementation-plans/progression-and-balance.md) program schedules this as its **slice 5** (balance catalog at `design/balance.md` + `balance-tuning` / `run-simulation` skills + the INV-20 `add-*` / advisor-planner-reviewer tooling updates). Two program deliverables give the catalog *live* data rather than a static table: the `IPowerBudgetSystem` estimator (slice 3) that projects any item/mob/loadout onto tier power bands, and the `Hedron.Sim` harness (slice 4) that validates real combat outcomes against those bands. This item is fulfilled when that program's slice 5 ships.
...
### 🔵 Progression anti-grind proxy → `IPowerBudgetSystem` (deferred from progression-substrate, slice 1)

The [`progression-substrate`](../implementation-plans/progression-substrate.md) slice needs a killer-vs-victim relative-power comparison for its combat-XP anti-grind guard, but the shared `IPowerBudgetSystem` power oracle is **slice 3** of the [`progression-and-balance`](../implementation-plans/progression-and-balance.md) program. Slice 1 therefore uses a **local proxy** inside `ProgressionSystem` — the sum of a combatant's effective attribute scores via `IStatSystem` — isolated behind a single private method. Slice 3 replaces that proxy with the `IPowerBudgetSystem` call (a ~one-line swap) and deletes it. Bounded, single-site acknowledged debt: it appears once, so it does not trip the INV-19 ≥3× "build the framework now" bar — the framework is already scheduled. Lands with slice 3.
...
The [`power-budget-inspector`](../implementation-plans/power-budget-inspector.md) slice (prog-3) gates `power`/`powerband` to admin/designer — raw balance internals (power scalars, band anchors) stay out of players' hands. Players still want a *rough* danger read before engaging ("how deadly is this fight?"). That is a deferred, **decoupled** player command `consider <mob>` that is a **thin consumer of the same `IPowerBudgetSystem` oracle**: gather the player's and the target's effective scores into two snapshots, call `Estimate`/`Classify` on each, and map the *relationship* (power ratio / band delta) to a coarse **diegetic label** ("trivial / even / dangerous / deadly") — never surfacing the raw numbers. The public `Estimate`/`Classify`/`BandAnchor` interface prog-3 ships already suffices with **no interface change**, so the capability is preserved without building it now (restraint — no in-slice consumer). If the danger-label logic later wants a shared home, add a small comparison helper on `IPowerBudgetSystem` *then*, when `consider` is the consumer. Lands when a player-facing threat-assessment verb is scheduled. Durable home on ship: [`../features/progression/`](../features/progression/) (the prog-3 plan disintegrates there). Recorded per the owner's decision to keep the big picture (prog-3 resolved slice Q3).
