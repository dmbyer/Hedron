...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation + item-value (12a) + mob-protection (12b) + shopping (12c) + `prog-1` progression substrate + `prog-2` Ascension + `prog-3` power model & balance inspector done; **now: Progression & Balance program — `prog-3b` power-model revision (Tier × Band + calibration + audit tooling), then `prog-4` simulation harness and `prog-5` agentic/balance-doc layer need `/new-plan`**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
- **Next — `prog-3b` Power-model revision.** A post-merge design conversation found `prog-3`'s one-axis Band model undershot the actual requirement: a two-axis **Tier (0–6, coarse gate) × Band (1–3, within each tier)** system, D&D-Challenge-Rating-style — given stats, classify (Tier, Band); given a target (Tier, Band), get a stat range to build toward. Scope: `Classify`/`BandAnchor` return both axes (~21 anchors); a new inverse target-range query; `TierBand` becomes a `Tier`+`Band` pair on both content kinds + both Blazor editors; soft, band-count-tolerance drift detection (an upgraded editor mismatch flag + a bulk audit report extending the Integrity sweep — never a hard blocker) that doubles as free power-level content reporting; recalibration of the placeholder `PowerBudgetConstants` against real targets; and a written extensibility principle for future power inputs (abilities/effects/speed/defense) so the oracle never regains a domain import. Also folds forward the one piece of `prog-5`'s INV-20 scope this revision itself depends on. **No plan yet** — frame with `/advise`/`/new-plan` against [`../implementation-plans/progression-and-balance.md`](../implementation-plans/progression-and-balance.md) Open questions 7–12. `prog-4` now depends on `3b`, not directly on `3`.
- **After that — `prog-4` Simulation harness.** `Hedron.Sim` batch-sim project (parameter sweeps, statistical reporting, combatant-policy seam), promoted CI regression invariants using `IPowerBudgetSystem` as the expected-outcome oracle (its 4th consumer). Depends on `prog-3b` landing first (the sim should validate against the revised two-axis oracle, not the one being replaced). No plan yet — frame with `/new-plan` against the program brief [`../implementation-plans/progression-and-balance.md`](../implementation-plans/progression-and-balance.md), scoped to the `prog-4` row + its Open Questions. `prog-5` (agentic + balance-doc layer) follows.
...
| prog-3b | **Power-model revision** — *program* | Two-axis `Classify` (Tier × Band 1–3) · inverse target-range query · `TierBand` → `Tier`+`Band` pair on content · band-count-tolerance audit tooling · calibration · power-model extensibility principle | 🔵 seed only → `/new-plan` (Open questions 7–12) |
| prog-4 | **Simulation harness** — *program* | `Hedron.Sim` batch sim · parameter sweeps · promoted CI invariants | 🔵 seed only → `/new-plan`, depends on `3b` |
...
The **`prog-*` rows are the [Progression & Balance program](../implementation-plans/progression-and-balance.md)** — a foundational program (Spine E progression + balance-observability tooling), advisor-framed at the program level. **`prog-1`**, **`prog-2`**, and **`prog-3`** shipped (see [`features/progression/progression.md`](../features/progression/progression.md), [`features/progression/ascension-system.md`](../features/progression/ascension-system.md), [`completed/progression-substrate.md`](completed/progression-substrate.md), [`completed/ascension.md`](completed/ascension.md), and [`completed/power-budget-inspector.md`](completed/power-budget-inspector.md)); a post-merge design conversation added **`prog-3b`** (a revision to `prog-3`'s Band model, not previously in the program map) as the immediate next slice. **`prog-3b`**…**`prog-5`** exist as the program map only — to build one, point `/new-plan` at the **program brief** (the seed), scoped to that slice's row + its Open Questions. Each slice then runs the normal loop (spec gate → implement → code gate → `sync-roadmap`).
