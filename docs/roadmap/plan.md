...
- **Progression expansion** — ✅ *partly landed early*: ability use and damage taken shipped as XP sources during Phase 5, which fired the ≥3-source threshold and promoted the trigger wiring to the advancement-rule table (see [`completed/progression-use-based-xp.md`](completed/progression-use-based-xp.md)). Still open here: exploration and objective sources (rows on the existing table), the ascension Objective gate + unlock grants, ability rank granting power, and extending the `progressionRate` sweep onto use-based accrual so the balance workbench can see it.
