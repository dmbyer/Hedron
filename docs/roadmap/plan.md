...
- **Next — `prog-4` Simulation harness.** `Hedron.Sim` batch-sim project (parameter sweeps, statistical reporting, combatant-policy seam), promoted CI regression invariants using `IPowerBudgetSystem` as the expected-outcome oracle (its 4th consumer). No plan yet — frame with `/new-plan` against the program brief [`../implementation-plans/progression-and-balance.md`](../implementation-plans/progression-and-balance.md), scoped to the `prog-4` row + its Open Questions. `prog-5` (agentic + balance-doc layer) follows.
...
| prog-3 | **Power model + balance inspector** — *program* | `IPowerBudgetSystem` oracle · tier power bands · admin `power`/`powerband` + Blazor editor readout · item tier-band + anti-grind-proxy rewire | ✅ done ([feature](../features/progression/progression.md) · [completed record](completed/power-budget-inspector.md)) |
...
The **`prog-*` rows are the [Progression & Balance program](../implementation-plans/progression-and-balance.md)** — a foundational 5-slice program (Spine E progression + balance-observability tooling), advisor-framed at the program level. **`prog-1`**, **`prog-2`**, and **`prog-3`** shipped (see [`features/progression/progression.md`](../features/progression/progression.md), [`features/progression/ascension-system.md`](../features/progression/ascension-system.md), [`completed/progression-substrate.md`](completed/progression-substrate.md), [`completed/ascension.md`](completed/ascension.md), and [`completed/power-budget-inspector.md`](completed/power-budget-inspector.md)); **`prog-4`**…**`prog-5`** exist as the program map only — to build one, point `/new-plan` at the **program brief** (the seed), scoped to that slice's row + its Open Questions. Each slice then runs the normal loop (spec gate → implement → code gate → `sync-roadmap`).
