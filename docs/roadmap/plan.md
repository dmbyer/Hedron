...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation done; **next: slice 12a — Item value**, then 12b — Mob protection, 12c — Shopping) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
**Phase 3 slice 12 — Shopping**, split by the `architecture-advisor` intake into three dependency-ordered slices: **12a — Item value** (the `ItemDataComponent.Value` substrate, precursor), **12b — Mob protection** (cross-cutting `ProtectionComponent`, independent/parallelizable), and **12c — Shopping** (the trade feature; depends on 12a, consumes 12b). Seeds at [`../implementation-plans/item-value.md`](../implementation-plans/item-value.md), [`mob-protection.md`](../implementation-plans/mob-protection.md), [`shopping.md`](../implementation-plans/shopping.md). Most recently shipped (advisor-initiated, off the numbered queue): **currency-foundation** — the economy substrate slice 12 builds on. Entities now carry an entity-keyed `WalletComponent` ledger driven by the single `IWalletSystem` mutation seam (`Deposit`/`TryWithdraw`/`CanAfford`/`Transfer`/`SetBalance`, pure per INV-5); a `CurrencyRegistry` (Spine F) defines currency families and their denomination ladders as data (launch family **Coin**: `100c = 10s = 1g`); mobs carry an opt-in `CurrencyLootComponent` (uniform `IRandom` roll) auto-awarded to the killer on `MobDiedEvent`; `score` shows balances and `setwallet` is the admin grant. The atomic `Transfer` primitive is exposed now for the downstream economy verbs — see [`../roadmap/completed/currency-foundation.md`](completed/currency-foundation.md) and the [economy feature](../features/economy/economy.md). Before it, **content-editor-integrity** (offline Blazor editor area filters, referential-integrity delete, Integrity sweep — see [`completed/content-editor-integrity.md`](completed/content-editor-integrity.md)) and **wearable-equipment-expansion** (worn-gear stat seam via `EquipmentStatBonus` + the INV-24 `EquipmentEffectContributor` — see [`completed/wearable-equipment-expansion.md`](completed/wearable-equipment-expansion.md)). The next *numbered* slice is **Shopping** ([`../implementation-plans/shopping.md`](../implementation-plans/shopping.md) — to be authored) — economy mechanics enabling players to buy/sell items from vendor NPCs, the first `Transfer` consumer (vendor till ↔ player wallet). It builds on items + inventory (slice 6), the mob model (slice 8), and the currency substrate (above); no ability or combat dependency.
...
| 12a | **Item value** (advisor seed) | `ItemDataComponent.Value` substrate (base-unit Coin) every economic price derives from | 🟢 next (precursor; planned — [`../implementation-plans/item-value.md`](../implementation-plans/item-value.md)) |
| 12b | **Mob protection** (advisor seed) | Cross-cutting `ProtectionComponent` (`Untargetable`/`EffectImmune`); combat + effect gates | 🟢 ready (independent of 12a — planned, [`../implementation-plans/mob-protection.md`](../implementation-plans/mob-protection.md)) |
