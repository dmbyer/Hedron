...
- **Next — `prog-4` Simulation harness.** `Hedron.Sim` batch-sim project (parameter sweeps, statistical reporting, combatant-policy seam), promoted CI regression invariants using `IPowerBudgetSystem` as the expected-outcome oracle (its next consumer). `prog-3b` has landed, so the sim validates against the revised two-axis oracle. No plan yet — frame with `/new-plan` against the program brief [`../implementation-plans/progression-and-balance.md`](../implementation-plans/progression-and-balance.md), scoped to the `prog-4` row + its Open Questions. `prog-5` (agentic + balance-doc layer) follows.
...
| prog-4 | **Simulation harness** — *program* | `Hedron.Sim` batch sim · parameter sweeps · promoted CI invariants | 🟢 ready — `/new-plan`, depends on `3b` (done) |
