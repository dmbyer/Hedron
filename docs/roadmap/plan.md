...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation + item-value (12a) + mob-protection (12b) + shopping (12c) + `prog-1` progression substrate + `prog-2` Ascension done; **now: Progression & Balance program — `prog-3` power model + balance inspector needs `/new-plan`; `prog-4`…`5` need `/new-plan`**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
- **Next — `prog-3` Power model + balance inspector.** `IPowerBudgetSystem` (core-tier shared oracle), tier power bands, `power`/`powerband` inspector + Blazor editor readout. Needs `/new-plan` (seed = the program brief, scoped to the `prog-3` row). `prog-4`…`prog-5` still need `/new-plan` after that.
...
| prog-3 | **Power model + balance inspector** — *program* | `IPowerBudgetSystem` oracle · tier power bands · `power`/`powerband` + Blazor editor readout | 🟢 next → `/new-plan` |
