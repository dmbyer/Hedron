...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation + item-value (12a) + mob-protection (12b) + shopping (12c) done; **now: Progression & Balance program — `prog-1` ready to `implement-plan`; `prog-2`…`5` need `/new-plan`**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
- **Ready to implement now — `prog-1` Progression substrate.** Full plan at [`../implementation-plans/progression-substrate.md`](../implementation-plans/progression-substrate.md); spec-review gate passed. Next: `implement-plan` → code gate → ship.
- **Needs `/new-plan` before implementing — `prog-2`…`prog-5`.** They exist only as the program map today. To build one: resolve that slice's Open Questions (listed in the program brief) with the owner, then point the `implementation-planner` at the **seed = the program brief** [`../implementation-plans/progression-and-balance.md`](../implementation-plans/progression-and-balance.md), scoped to that slice's row. The planner writes a new `implementation-plans/<slice>.md`; then the spec gate runs.
...
| prog-1 | **Progression substrate** — *Progression & Balance program* | Per-track XP · contribute-on-read into `IStatSystem` (INV-24) · use-driven accrual · `progress` inspector | 🟢 **plan ready + spec-passed → `implement-plan`** ([plan](../implementation-plans/progression-substrate.md)) |
| prog-2 | **Ascension / character tier** — *program* | Character-wide Tier scalar · additive power baseline · tier-up gate · overlapping content bands | 🔵 seed only → `/new-plan` ([brief](../implementation-plans/progression-and-balance.md)) |
| prog-3 | **Power model + balance inspector** — *program* | `IPowerBudgetSystem` oracle · tier power bands · `power`/`powerband` + Blazor editor readout | 🔵 seed only → `/new-plan` |
| prog-4 | **Simulation harness** — *program* | `Hedron.Sim` batch sim · parameter sweeps · promoted CI invariants | 🔵 seed only → `/new-plan` |
...
The **`prog-*` rows are the [Progression & Balance program](../implementation-plans/progression-and-balance.md)** — a foundational 5-slice program (Spine E progression + balance-observability tooling), advisor-framed at the program level. **`prog-1`** has a full, spec-passed plan ([`progression-substrate.md`](../implementation-plans/progression-substrate.md)) ready for `implement-plan`; **`prog-2`…`prog-5`** exist as the program map only — to build one, point `/new-plan` at the **program brief** (the seed), scoped to that slice's row + its Open Questions. Each slice then runs the normal loop (spec gate → implement → code gate → `sync-roadmap`).
