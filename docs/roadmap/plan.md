...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation + item-value (12a) + mob-protection (12b) + shopping (12c) + `prog-1` progression substrate done; **now: Progression & Balance program — `prog-2`…`5` need `/new-plan`**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
- **Next — needs `/new-plan` before implementing: `prog-2` Ascension (character-wide tier).** Resolve its Open Questions (additive-baseline vs rescale-on-ascend; specialization-on-ascend selection; tier-baseline home) with the owner, then point the `implementation-planner` at the **seed = the program brief** [`../implementation-plans/progression-and-balance.md`](../implementation-plans/progression-and-balance.md), scoped to the `prog-2` row. The planner writes `implementation-plans/<slice>.md`; then the spec gate runs. `prog-3`…`prog-5` follow the same path once `prog-2` ships.
...
The **`prog-*` rows are the [Progression & Balance program](../implementation-plans/progression-and-balance.md)** — a foundational 5-slice program (Spine E progression + balance-observability tooling), advisor-framed at the program level. **`prog-1`** shipped (see [`features/progression/progression.md`](../features/progression/progression.md) and [`completed/progression-substrate.md`](completed/progression-substrate.md)); **`prog-2`…`prog-5`** exist as the program map only — to build one, point `/new-plan` at the **program brief** (the seed), scoped to that slice's row + its Open Questions. Each slice then runs the normal loop (spec gate → implement → code gate → `sync-roadmap`).
