...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation + item-value (12a) + mob-protection (12b) + shopping (12c) done; **next: slice 13 — Crafting / potions**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
**Phase 3 slice 13 — Crafting / potions** is the next numbered slice (content depth on top of items + inventory). It has no use-case doc yet; frame it with `/advise` before planning.
...
| 13 | Crafting, potions | Content depth | 🟢 next (ready after 6) |
