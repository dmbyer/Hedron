...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation + item-value (12a) + mob-protection (12b) + shopping (12c) + `prog-1` progression substrate + `prog-2` Ascension done; **now: Progression & Balance program — `prog-3` power model + balance inspector plan spec-passed (ready to implement); `prog-4`…`5` need `/new-plan`**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
- **Next — `prog-3` Power model + balance inspector.** `IPowerBudgetSystem` (core-tier shared oracle — snapshot input, weighted `ScoreId` table, tier bands derived from the Ascension baseline), admin-gated `power`/`powerband` inspectors, the Blazor editor readout (the primary designer observability surface), an authored item tier-band tag, and the anti-grind-proxy rewire (the 3rd in-slice consumer that retires the documented tech debt). **Plan spec-passed and ready to implement** — [`../implementation-plans/power-budget-inspector.md`](../implementation-plans/power-budget-inspector.md). `prog-4`…`prog-5` still need `/new-plan` after that.
...
| prog-3 | **Power model + balance inspector** — *program* | `IPowerBudgetSystem` oracle · tier power bands · admin `power`/`powerband` + Blazor editor readout · item tier-band + anti-grind-proxy rewire | 🟢 ready — [plan](../implementation-plans/power-budget-inspector.md) spec-passed |
...
The **`prog-*` rows are the [Progression & Balance program](../implementation-plans/progression-and-balance.md)** — a foundational 5-slice program (Spine E progression + balance-observability tooling), advisor-framed at the program level. **`prog-1`** and **`prog-2`** shipped (see [`features/progression/progression.md`](../features/progression/progression.md), [`features/progression/ascension-system.md`](../features/progression/ascension-system.md), [`completed/progression-substrate.md`](completed/progression-substrate.md), and [`completed/ascension.md`](completed/ascension.md)); **`prog-3`…`prog-5`** exist as the program map only — to build one, point `/new-plan` at the **program brief** (the seed), scoped to that slice's row + its Open Questions. Each slice then runs the normal loop (spec gate → implement → code gate → `sync-roadmap`).
