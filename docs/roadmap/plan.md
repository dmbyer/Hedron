...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity done; **next: slice 12 — Shopping**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
**Phase 3 slice 12 — Shopping.** Most recently shipped (advisor-initiated, off the numbered queue): **content-editor-integrity** — the offline Blazor content editor gained area filtering, lookup-driven selection fields, and a dark-readable theme, plus a declared-edge `IContentReferenceIndex` backing `DeleteAsync` cascade-clear (file-only, INV-22/23), warn-but-allow `SaveAsync`, an Integrity broken-link sweep page, and bidirectional exit linking (`Direction.Opposite`) — see [`../roadmap/completed/content-editor-integrity.md`](completed/content-editor-integrity.md). Before it, **wearable-equipment-expansion** — worn gear now drives effective stats as data via `EquipmentStatBonus(ScoreId, int)` rows on `ItemDataComponent` and a new `EquipmentEffectContributor` (the INV-24 effect contributor seam, derived on read) folded by `IStatSystem.Get(AttackPower|Defense)`; the flat `ItemDataComponent.DamageBonus` was removed and migrated onto that path (combat repointed to `Get(...)`); `WornSlot` gained 9 values for a full armor suit; authoring via `setitem bonus`/`clearbonus` + YAML `statBonuses:` + the Blazor row editor — see [`../roadmap/completed/wearable-equipment-expansion.md`](completed/wearable-equipment-expansion.md). Before it, the **content-tooling platform** landed (shared content-definition layer `IContentDefinitionCatalog` + callable `IContentValidator`, split hosted-service registration, a headless `generate` run-mode, and the in-process Blazor editor `Hedron.Web` over all four content kinds — see [`../roadmap/completed/content-tooling-platform.md`](completed/content-tooling-platform.md)). The next *numbered* slice is **Shopping** ([`../implementation-plans/shopping.md`](../implementation-plans/shopping.md) — to be authored) — economy mechanics enabling players to buy/sell items from vendor NPCs. This slice builds on the items + inventory substrate (slice 6) and the mob model (slice 8); no ability or combat dependency.
