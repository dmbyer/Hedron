...
| **3 — Vertical slices** | 🟡 in progress (slices 1–11-d + output-batching + area-model + content-tooling + wearable-equipment-expansion + content-editor-integrity + currency-foundation + item-value (12a) + mob-protection (12b) + shopping (12c) + `prog-1` progression substrate done; **now: Progression & Balance program — `prog-2` planned/spec-passed (ready to implement); `prog-3`…`5` need `/new-plan`**) | per-slice docs in [`../implementation-plans/`](../implementation-plans/); see [Slice queue](#slice-queue) |
...
- **Planned & spec-passed — ready to implement: `prog-2` Ascension (character-wide tier).** Plan: [`../implementation-plans/ascension.md`](../implementation-plans/ascension.md) (spec-review gate: APPROVE WITH NITS; nits applied). Owner-resolved decisions (2026-07-05): additive power baseline (no XP reset) · new `Core/Modules/Ascension/` module (`AscensionComponent` + `IAscensionSystem`) · tier baseline rides the existing `IEffectContributor` port (4th registrant, `AscensionEffectContributor`) · admin `ascend` + `CanAscend`/`TryAscend` gate (real Ascension-Objective gate deferred) · **mobs-only** tier-band tag · **seam-only** unlock records (empty table). Next: `implement-plan` against the three work packages. `prog-3`…`prog-5` still need `/new-plan` (seed = the program brief, scoped to each row).
...
| prog-2 | **Ascension / character tier** — *program* | Character-wide Tier scalar · additive power baseline · tier-up gate · overlapping content bands | 🟢 ready — plan [`ascension.md`](../implementation-plans/ascension.md) spec-passed |
...
The **`prog-*` rows are the [Progression & Balance program](../implementation-plans/progression-and-balance.md)** — a foundational 5-slice program (Spine E progression + balance-observability tooling), advisor-framed at the program level. **`prog-1`** shipped (see [`features/progression/progression.md`](../features/progression/progression.md) and [`completed/progression-substrate.md`](completed/progression-substrate.md)); **`prog-2`** is planned & spec-passed ([`ascension.md`](../implementation-plans/ascension.md) — ready to implement); **`prog-3`…`prog-5`** exist as the program map only — to build one, point `/new-plan` at the **program brief** (the seed), scoped to that slice's row + its Open Questions. Each slice then runs the normal loop (spec gate → implement → code gate → `sync-roadmap`).
