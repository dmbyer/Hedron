...
- `Hedron.Tests/Registry/RegistryValidationTests.cs` (addition, code-review follow-up) — a coordinate-collision warning exercised at the `RegistryValidationBootstrap.StartAsync` level (not just `ContentValidator.ValidateRegistry`), asserting boot does not throw (→ postcondition 9, bootstrap-level coverage).
- `dotnet test` green: 1266 tests (was 1212 before this slice).
