...
> Implemented on `master` (no PR number yet — uncommitted working-tree changes as of this ledger entry). The full feature spec lives in [`../../use-cases/output-framework.md`](../../use-cases/output-framework.md). This file records the as-built state and any deviations from the spec.
