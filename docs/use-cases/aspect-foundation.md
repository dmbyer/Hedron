...
Three packages. **WP-1 lands first** (pure refactor, unblocks nothing functionally but de-risks the abstraction). **WP-2 depends on WP-1**; **WP-3 depends on WP-2**. WP-2 and WP-3 are sequential because the inspector and validator both need `AspectRegistry` + the migrated `Aspect` fields to exist. The **primary agent runs `architecture-reviewer` (code mode) across the combined diff** after all three land.
...
### WP-2 — Aspect feature + combat integration
- **Scope:** `AspectId`, `AspectCategory`, `AspectDefinition`, `AspectComposition`, `AspectRegistry` (born on WP-1's base). `IAspectSystem` / `AspectSystem` (`Resolve` / `Affinity` / `Resist`). `AspectAffinitiesComponent` (`[Persistent]`, base affinity composition + base per-aspect resist map). Migrate `AbilityDefinition.Aspect` → `AspectComposition`. Thread the composition through `CombatSystem` and call `Resolve` before `SetCurrentHp`; enrich `CombatRoundResult` + the ability-strike result + their events. Attach `AspectAffinitiesComponent` in `AccountSystem.CreateCharacterAsync`. Author the starter `AspectRegistry` rows. Register the per-aspect resist contributor(s) for the `IAspectSystem.Resist` fold.
- **Files:** `Core/Modules/Aspects/` (new module: `AspectsModule.cs`, `AspectId.cs`, `AspectDefinition.cs`, `AspectComposition.cs`, `AspectRegistry.cs`, `Systems/IAspectSystem.cs`, `Systems/AspectSystem.cs`); `Core/ECS/Components/AspectAffinitiesComponent.cs` (new, cross-cutting); `Core/Modules/Abilities/AbilityDefinition.cs`, `AbilityRegistry.cs` (changed — `Aspect` type + rows); `Core/Modules/Combat/Systems/CombatSystem.cs`, `Core/Modules/Combat/Events/CombatRoundEvent.cs`, the ability-strike event, `CombatTickHandler.cs`, `AbilityInvocationPipeline` (changed); `Core/Modules/Account/Systems/AccountSystem.cs` (changed); `Server/Program.cs` (register `AddAspectsModule`).
- **Depends on:** WP-1.
- **Out of scope:** the `defs` inspector and the validation sweep (WP-3); aspect-unique ability/effect riders on `AspectDefinition` (shape-for-later); the `EffectParams.Aspect` migration **may be deferred** if it expands the diff — if so, file a one-line backlog note and leave the `string?` stub with a `// TODO migrate` (do not consume it); aspect oppositions (R2: field declared, no pairs).
- **Exit criterion:** an aspect-typed strike (e.g. a fire-typed ability vs. a target with fire resistance) deals visibly reduced damage vs. an un-resisted target; a melee round with an attacker carrying an affinity composition deals correspondingly boosted damage; `score`/combat output renders the damage type; new characters carry `AspectAffinitiesComponent`; round-trips through save/load by aspect **name** (not ordinal).

### WP-3 — Tooling (validation + `defs`)
...
- **Depends on:** WP-2 (needs `AspectRegistry` + the migrated `Aspect` fields to validate/inspect).
...
- **Combat / damage path:** **Gap exposed → resolved in-slice.** Damage is a bare `int` today; making it `(magnitude, AspectComposition)` and inserting `IAspectSystem.Resolve` is a real change to the combat tick, specified in WP-2 and the combat-round flow. Not hand-rolled per aspect — one resolution function, no `FireSystem` (the spine invariant).
...
- **flow-18 (Combat round pulse) — modified.** The strike now constructs `(magnitude, AspectComposition)` and calls `IAspectSystem.Resolve` before `SetCurrentHp`; the published `CombatRoundEvent` carries the composition. WP-2's PR updates flow-18 (body + mermaid: add the `IAspectSystem.Resolve` participant/step). flow-24/26 (ability activation / offensive ability) similarly reference the strike path through `ResolveAbilityStrike` — update the relevant step text where the magnitude is now aspect-resolved.
- **flow-01 (Server startup) — modified.** Adds the registry-population guarantee (instance rows loaded at DI construction) and the new `RegistryValidationInitiator` step (fail-fast sweep after `WorldContentReadyEvent`, before the listener serves). WP-1 (population) + WP-3 (validation) update flow-01 (body + mermaid: add the validation Initiator box ordered after `WorldContentBootstrap`).
...
- `reference/commands.md`: `defs` admin command (WP-3).
