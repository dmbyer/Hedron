...
- **Validation before exit:** emitted YAML is validated before the run reports success by calling the shared `IRegistryValidator` (Resolved Decision 2). `GenerationResult.ValidationErrors` is non-empty ⇒ non-zero exit; the report lists each offending file.
...
6. **Validate.** Each written file is validated through the shared `IRegistryValidator` (Resolved Decision 2). Failures accumulate into `GenerationResult.ValidationErrors`.
...
| `IRegistryValidator` | Reused (owned by T2) — precondition | `Core/Modules/Authoring/` or `Core/Systems/` (T2 decides) |
...
**Scope:** the `Server` run-mode branch that parses `generate --profile <path> [--seed N]`, composes services without gameplay hosted services, loads + deserializes the profile YAML, runs one `GenerateAsync`, validates emitted files via the shared `IRegistryValidator`, prints the summary, and sets the exit code. Flows + reference updates. **Depends on the shared `IRegistryValidator` (T2) having landed (Resolved Decision 2).**
...
**Out of scope:** admin verb caller, applying generated content to a live world (`reload`), the shared `IRegistryValidator` factoring (T2), procedural in-game generation.
...
- [`content-tooling-platform.md`](content-tooling-platform.md) — the architecture-advisor platform brief this slice (T1) extends; owns the shared seam rationale, family disposition, and resolved decisions. The Blazor authoring track (T2) and the shared `IRegistryValidator` live there.
