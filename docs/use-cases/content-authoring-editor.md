...
9. ✅ **Landed in S0** — `Hedron.Tests/Composition/HostCompositionTests.AddGameplayHostedServices_RegistersFullGameplayHostedServiceSet` asserts `AddGameplayHostedServices` still registers all six hosted services after the split. Guards against silently dropping a gameplay service from the telnet host. (The `Hedron.Web` bootstraps-only set is exercised by launching the host, not unit-tested — host plumbing, skip-tier.)
